...
const SupportAdmin = () => {
  ...
  const loadTickets = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || 'Impossible de charger les demandes de support.');
    } finally {
      setLoading(false);
    }
  }, [statusFilter]);
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">{tickets.length} demande{tickets.length > 1 ? 's' : ''}{statusFilter ? ` · ${statusFilter}` : ''}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={loadTickets} className="rounded-xl border border-gray-200 p-2 text-gray-600 hover:bg-gray-50" aria-label="Rafraîchir">
            <RefreshCcw size={16} />
          </button>
        </div>
      </div>
            ...
            <div key={ticket.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                <div>
                  ...
                  <div className="text-xs text-gray-500">{formatDate(ticket.created_at)} · {ticket.channel === 'direct' ? 'Contact direct' : 'Assistant robot'}</div>
                </div>
                <select
                  value={ticket.status}
              ...
              <div className="mb-2">
                <span className="text-xs font-semibold uppercase text-gray-400">Motif</span>
