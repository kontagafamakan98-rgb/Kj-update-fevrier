#!/usr/bin/env python3
"""
AUDIT COMPLET KOJO - Test Backend CORRIGÉ selon demande française
Détection et correction de bugs pour l'Afrique de l'Ouest

Test des 5 domaines critiques avec endpoints réels:
...
5. EMPLOIS & PAIEMENTS (via registration avec payment accounts)
"""
...
class KojoFrenchAuditCorrectedTester:
    def __init__(self, base_url="https://kojo-work.preview.emergentagent.com/api"):
        ...
        print("🇫🇷 AUDIT COMPLET KOJO - BACKEND AFRIQUE DE L'OUEST (CORRIGÉ)")
        print("=" * 70)
    ...
    def test_1_geolocalisation(self):
        """1. GÉOLOCALISATION (Client & Travailleur) - ENDPOINTS RÉELS"""
        print("\n" + "="*70)
        print("1. TEST GÉOLOCALISATION - AFRIQUE DE L'OUEST (ENDPOINTS RÉELS)")
        print("="*70)
        ...
        # Test POST /api/geolocation/validate-phone (CORRIGÉ - avec body JSON)
        for phone, pays in pays_tests:
            self.run_test(
                ...
                200,
                data={"phone": phone, "country": pays.lower().replace(" ", "_").replace("'", "_")}
            )
    ...
    def test_4_emplois_paiements(self):
        """5. EMPLOIS & PAIEMENTS (via registration avec payment accounts)"""
        print("\n" + "="*70)
        print("4. TEST EMPLOIS & PAIEMENTS (VIA REGISTRATION AVEC PAYMENT ACCOUNTS)")
        print("="*70)
        ...
        # Test validation Orange Money et Wave via registration avec payment accounts
        timestamp = datetime.now().strftime('%H%M%S')
        
        # Test Orange Money pour les 4 pays via registration
        orange_money_tests = [
            ("+221771234567", "senegal", "Sénégal"),
            ("+223701234567", "mali", "Mali"),
            ("+226701234567", "burkina_faso", "Burkina Faso"), 
            ("+225071234567", "ivory_coast", "Côte d'Ivoire")
        ]
        
        for i, (phone, country, pays_nom) in enumerate(orange_money_tests):
            user_data = {
                "email": f"orange_test_{i}_{timestamp}@test.com",
                "password": "TestPass123!",
                "first_name": "Test",
                "last_name": f"Orange{pays_nom}",
                "phone": phone,
                "user_type": "client",
                "country": country,
                "preferred_language": "fr",
            ...
            self.run_test(
                f"Validation Orange Money {pays_nom} (via registration)",
                "POST",
        ...
        # Test Wave pour les 4 pays via registration
        wave_tests = [
            ("+221771234567", "senegal", "Sénégal"),
            ("+223701234567", "mali", "Mali"),
            ("+226701234567", "burkina_faso", "Burkina Faso"),
        ...
        for i, (phone, country, pays_nom) in enumerate(wave_tests):
            user_data = {
                "email": f"wave_test_{i}_{timestamp}@test.com",
                "password": "TestPass123!",
                "first_name": "Test",
                "last_name": f"Wave{pays_nom}",
                "phone": phone,
                "user_type": "client",
                "country": country,
                "preferred_language": "fr",
            ...
            self.run_test(
                f"Validation Wave {pays_nom} (via registration)",
                "POST",
                "auth/register-verified",
                200,
                data=user_data
    ...
    def run_complete_audit(self):
        ...
        print("🚀 DÉMARRAGE AUDIT COMPLET KOJO BACKEND (CORRIGÉ)")
        print()
        ...
        print("\n" + "="*70)
        print("📊 RÉSULTATS AUDIT COMPLET KOJO (CORRIGÉ)")
        print("="*70)
        ...
        print("\n🇫🇷 AUDIT COMPLET KOJO TERMINÉ (CORRIGÉ)")
        return self.tests_passed / self.tests_run >= 0.8

if __name__ == "__main__":
    tester = KojoFrenchAuditCorrectedTester()
