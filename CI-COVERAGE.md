...
| **14. Le budget du coût du document n’est pas un gate Lighthouse, et ses bornes viennent d’un poste partagé** | `e2e/style-layout-document.spec.js` borne le « Style & Layout » du DOCUMENT pré-rendu (coquille seule, bundle d’entrée bloqué) : il ne dit rien de la part de React après montage, du premier écran peint ni du CLS, qui ont leurs propres sondes et leurs propres tables. Les relevés sont des MINIMA mesurés sur un poste de développement PARTAGÉ (Windows, 23 Chrome utilisateur) : la borne haute (×1,5 ou +80 ms mobile / +40 ms desktop) est un garde-fou de CATASTROPHE, et elle n’a pas pu être franchie par mutation — les quatre mutations mesurées (contenu ajouté en tête ou en queue) sont BÉNIGNES par mesure (+0 à +3 %). | Ce qui mord est la borne BASSE (×0,6 du relevé) : un document qui devient beaucoup moins cher a perdu sa matière, et la mutation le prouve (accueil amputé de ses neuf dernières sections : `/ — mobile` ET `/ — desktop` rouges, restauration vérifiée à l’empreinte SHA-1). La table est unitairement éprouvée (`scripts/__tests__/style-layout-budgets.test.js`, 8 cas) et deux mutations du registre la prouvent ; une route ajoutée à `src/config/page-meta.js` sans budget fait rougir un cas dédié. La mesure est rejouable localement : `cd frontend && npx playwright test e2e/style-layout-document.spec.js`. |
