...
| `workflow_dispatch` de `.github/workflows/lighthouse-mesure-locale.yml` | **oui**, 1 job — et **sans aucun budget** : la mesure Lighthouse locale sur les trois routes (`/,/jobs,/contact`), ses LHR run par run publiés en artifact. Rien n'y échoue sur une performance ; ce qui bloque reste `lighthouse-ci` (`lighthouserc*.cjs`) |
...
| **Frontend tests + build (Node/Vite)** | `vitest run`, `audit_api_returns.cjs` strict, `vite build` — **qui refuse déjà une page de route publique sans métadonnées** (plugin `require-page-meta`, §3 F12), donc avant même d'écrire un artefact —, puis **14 gardes sur les artefacts et les sources** (shells de pré-rendu, provenance des textes, ressources tierces des coquilles, sélecteurs CSS sans porteur, métadonnées runtime vs build, contrat OG offline, split pack2, hygiène i18n, split `services/api`, cartes OG, famille d'icônes, manifeste PWA, routage SPA, shell d'accueil/SEO, budgets de bundle, hygiène des branches distantes, **composants à état partagé et écouteurs à ref locale**, **contrôles interactifs démontés pendant l'appui**), et **sur `main`** la vérification que le frontend **SERVI** annonce la révision de ce commit (`check-deployed-revision.js`, §3 F19). | Non, sur son périmètre. Aucun seuil de couverture : supprimer des tests reste vert (§7). |
...
**Depuis le 24/09/2026, une passe desktop garde le TBT d'un poste de bureau**,
avec un plafond **par route** (≤ 200 ms sur l'accueil, ≤ 150 ms sur `/jobs`,
meilleur des 3 runs) — `frontend/lighthouserc.desktop.cjs`, exécutée par le même
job. Le socle ci-dessus ne dit rien de l'interactivité sur un poste de bureau : il
est mesuré en condition **mobile** simulée (CPU ×4 + 4G), et ses plafonds TBT
(1 200 / 1 600 ms) sont dimensionnés pour un runner partagé, pas pour un seuil
d'audit. Or les `settings` d'un collect valent pour **toutes** ses URLs : une même
passe ne peut pas être mobile pour 13 pages et desktop pour une. La passe desktop
n'audite donc que les deux pages dont un TBT desktop est MESURÉ — l'accueil et
`/jobs` — et n'asserte, par page, que deux choses : le TBT desktop et le plafond
CLS **mesuré** de la route. Les deux viennent d'une **table mesurée** qui refuse
une page sans relevé (le chargement de la config échoue alors : c'est le refus
éprouvé pour l'ajout de `/jobs`, en TBT comme en CLS).

Chaque plafond est adossé à la distribution de SA page :

| route | plafond | mesure |
|---|---|---|
| `/` | 200 ms | seuil de l'audit qui a ouvert le chantier ; 0 / 0 / 0 ms sur le déploiement réel (score 96-98), 0 / 0 / 0 ms en repli local au repos, **0 / 0 / 15 ms sous 4 boucles CPU sur 8 cœurs** — plus de 13× la pire mesure |
| `/jobs` | 150 ms | **27 runs desktop à 0 ms**, quatre conditions : pile de la CI, 4 boucles CPU, bord de CDN froid (+120 ms par actif), 7 boucles CPU sur 8 cœurs (famine) — aucune tâche au-dessus du seuil de « tâche longue » dans aucun run |

Le plafond de `/jobs` n'est pas une marge appliquée à 0 ms (tout nombre aurait
alors été « justifié ») mais une **borne haute** du coût de la page : sous famine
CPU, la totalité de son travail de style et de mise en page vaut 149 ms, donc
~99 ms de blocage même si tout coalesçait en une seule tâche. 150 ms couvre cette
borne avec 1,5× de marge — et reste **7× sous** le mode froid mesuré avant
correctif (voir ci-dessous), c'est-à-dire qu'il continue de l'attraper s'il
devenait permanent. C'est ce qu'un seuil commun ne pouvait pas faire : recopié de
l'accueil, il ne disait rien du pire cas de `/jobs`, et l'accueil porte en plus le
cadre Maps (la plus grosse passe de `Layout` du site, 674-728 ms relevées à
4× CPU), donc sa borne haute n'est pas comparable.

Le mode « mauvais » historique de `/jobs` (premier run à froid d'un déploiement
frais) est le sujet d'un correctif du 24/09/2026 : la première visite montait le
squelette **deux fois** (fallback Suspense de la route, puis la page elle-même),
soit une mise en page complète du document en plus, sur un document gonflé à
2931 px et porté par 108 animations. Après correctif, la condition « bord de CDN
froid » (+120 ms sur chaque actif, mesurée 5 fois) donne 0 ms de TBT et un
`Style & Layout` identique à celui d'une machine au repos ; le pire run à chaud
d'un déploiement reste 8 ms.

Preuve d'échec :
`frontend/scripts/__tests__/lhci-desktop-tbt.test.js`, qui rejoue les verdicts sur
le moteur d'assertions de lhci — 477 ms (la valeur de l'audit) et un millimètre
au-dessus de chaque plafond rougissent **sur la route concernée**, les valeurs
mesurées passent, la borne haute de `/jobs` (99 ms) passe, et `[1076, 10, 10]`
comme `[250, 10, 10]` passent — c'est le meilleur des 3 runs qui décide. Et
bout en bout avec le vrai CLI, sur 6 rapports réels (3 runs par route) :

```
mesures réelles (0 ms ×6)                      → exit 0  (« 2 URL(s), 6 total run(s) »)
/jobs muté à 160 ms, plafond PAR ROUTE         → exit 1  (expected: <=150, found: 160)
la MÊME mesure, config au seuil commun 200     → exit 0  ← ce que l'ancienne règle laissait passer
/jobs muté à 151 ms                            → exit 1
accueil muté à 201 ms                          → exit 1  (expected: <=200)
```
...
Ce que ce garde NE PEUT PAS dire, et qui a désormais sa sonde : quand la CI rougit
sur un CLS, elle donne un chiffre par route (médiane de 3 runs) sans nommer
l'élément déplacé ni dire si le décalage vient de la coquille ou du montage de
React. `e2e/cls-coquille-react.spec.js` (23 cas : 11 routes × 2 tailles + le refus
d'une route sans budget) mesure le CLS de Chrome (fenêtre de session) sur DEUX
navigations — bundle d'entrée bloqué, puis réelle — à 412×823 et 1350×940, publie
les sources de chaque décalage (nœud, rectangle avant, rectangle après) et
compare au budget LU dans `scripts/lhci-cls-budgets.cjs`. Relevé du 25/09/2026 :
coquille 0,0000 sur les 22 (0 décalage) ; montage de React 0,0088 sur /register
mobile — la valeur exacte des 27 runs de la CI — 0,0296 sur /payment mobile
(redirection), 0,0023 sur /support desktop, 0,0000 ailleurs. Fait mesuré au
passage : le montage en lui-même n'enregistre aucun décalage (nœuds neufs), donc
un budget CLS garde les réactions qui le SUIVENT — c'est pour cela que cette
sonde, contrairement aux autres, ne gèle pas le premier rendu. Les sondes
d'atelier de #19/#20 (probe 1280×4000, probe 412×823) sont remplacées par cette
sonde permanente.
...
| Le CHROME des coquilles (`vite-plugins/prerender/app-chrome.js` — un module de build, pas un garde : il enveloppe le corps des douze coquilles pré-rendues) | `scripts/__tests__/check-app-chrome.test.js` (5 tests mesurés le 25/09/2026 : le chrome est la COPIE de celui de `src/App.js` — les trois conteneurs et leurs classes sont lus dans App.js, donc l’habillage suit l’app au lieu de dériver ; la navbar réserve **65 px** (`border-b` autour d’un `h-16`, mesuré à 412 ET 1350 px — un `h-16 … border-b` seul en fait 64, tout le contenu démarre 1 px plus haut) ; le corps est rendu DANS `main` ; aucune coquille ne recopie le chrome ni la navbar historique `h-16 bg-white border-b border-gray-200` ; et les DEUX injections du build (accueil et routes) passent par `chromeDePage`) |
...
| `check-shell-remote-resources.js` (+ la règle qu'il exécute, `shell-remote-resources.js`) | `scripts/__tests__/check-shell-remote-resources.test.js` (23 cas mesurés le 25/09/2026, deux niveaux : la RÈGLE sur corpus et coquilles fixture — ce qui déclenche un téléchargement (sept familles de balises, `srcset` candidat par candidat, `style="… url(…)"`), ce qui n'en déclenche pas (navigation, JSON-LD), le sort des `rel` (ressource / sans téléchargement / NON CLASSÉ, `rel` à deux mots), ce qui est distant (relatif, `data:`/`blob:`, les deux origines du site, relatif au protocole, `http://` sur notre hôte, hôte prefixé, imageSet, `@import` en relais), l'entité `&quot;` résolue — et le GARDE exécuté en sous-processus sur une arborescence de build fixture : iframe tierce nommée, `@font-face` sur CDN, `rel` non classé, build sans coquille, plancher de lecture non atteint, plus l'ARBRE RÉEL — 189 fichiers de `src/`, `vite-plugins/` et `index.html` sans une seule ressource distante, le contrôle qui rougit AVANT le build) ; trois mutations rejouées le 25/09/2026 sur le garde réel (verdict neutralisé → trois cas rouges nommés ; appel de lecture neutralisé → iframe tierce non refusée ; plancher neutralisé → un lecteur cassé passerait pour un vert), restaurations vérifiées à l'empreinte SHA-1 identique |
| `check-composants-etat-partage-ecouteurs.js` (+ la règle `composants-etat-partage-ecouteurs.js`) | `scripts/__tests__/check-composants-etat-partage-ecouteurs.test.js` (7 cas mesurés le 25/09/2026, deux niveaux : la RÈGLE par analyse AST Babel — détection du composant bugué historique (`useNotifications` + `useRef` + `document.addEventListener('mousedown')` fermant le panneau), tolérance d'un état local (`useState`), tolérance d'un composant ancré à ref partagée (`ancre.contains`) — et le GARDE en sous-processus : sortie 0 sur fixture propre, sortie 1 nommant le composant, fichier, ligne, ref et action sur fixture buguée, refus d'un répertoire sous le plancher (< 30 fichiers), et vérification de l'arbre réel : 135 fichiers JSX/JS scannés, 0 violation) ; deux mutations déclarées dans `guard-proofs.json` et rejouées par `check-guard-mutations.py` (verdict neutralisé → cas fixture buguée rouge ; plancher neutralisé → cas refus plancher rouge) |
| `check-appui-demontage-controles.js` (+ la règle `appui-demontage-controles.js`) | `scripts/__tests__/check-appui-demontage-controles.test.js` (12 cas mesurés le 26/09/2026, deux niveaux : la RÈGLE par analyse AST Babel — contrôle dont l'appui (`onMouseDown`/`onPointerDown`/`onTouchStart`) écrit un état local qui conditionne son rendu, gestionnaire d'appui porté par un ANCÊTRE (bouillonnement), retour anticipé `if (!ouvert) return null`, tolérance d'un état non lié au rendu du contrôle, tolérance de la fermeture par prop (`ConfirmModal`, `onCancel?.()`), contrôle sans gestionnaire d'appui ignoré, comptage des contrôles — et le GARDE en sous-processus : sortie 0 sur fixture propre, sortie 1 nommant le fichier, le contrôle, l'événement et l'état sur fixture buguée, refus des DEUX planchers (fichiers < 30, contrôles < 100), et vérification de l'arbre réel : 135 fichiers, 266 contrôles interactifs, 0 violation) ; deux mutations déclarées dans `guard-proofs.json` et rejouées par `check-guard-mutations.py` (détection neutralisée → cas fixture buguée rouge ; plancher de contrôles neutralisé → cas refus plancher rouge) |
| `check-prerender-shells.js` | `scripts/__tests__/check-prerender-shells.test.js` (45 tests mesurés le 25/09/2026 : le garde EXÉCUTÉ en sous-processus sur une arborescence de build fixture conforme — vérifiée verte — puis onze régressions injectées une à une : `#root` vidé, modulepreload du chunk Home perdu, h1 de login retiré, champ de formulaire register retiré, `og:image` d'une coquille retiré (carte LUE dans la table unique), coquille absente du build, page pré-rendue routée d'un seul côté, catch-all `/(.*)` réintroduit, `vercel.json` illisible, **carte Google republiée en `iframe` dans `contact.html`** (le premier écran chargeait ~300 Ko de tiers : LCP mobile 4143-4397 ms simulés, scores 81-85 — la coquille publie désormais le contrôle déclaré par le plan, `mapButtonKey` / `mapFrameClass`), **chrome de l’app retiré de la coquille `/jobs`** (le corps ne commence plus par `.App` + navbar + `main.flex-1` : la coquille n’hérite plus des mêmes styles que la page — mesuré, la 2e ligne du paragraphe d’intro passait de x=67.000 à x=218.578 et l’aire LCP du MÊME texte de 36 002 à 36 049 px², ce qui suffisait à ré-élire la peinture de React à 2,5 s, 3 runs sur 3 en desktop) — chacune exige `exit 1` ET le message qui nomme le coupable) |
...
| **6. Périmètre navigateur Playwright E2E** | La suite E2E tourne sous Chromium headless uniquement ; elle ne couvre ni WebKit/Safari, ni Firefox, ni les terminaux mobiles physiques. | Valide 9 parcours clés (GPS, formulaires, auth, centre de notifications, LCP de /contact, géométrie du LCP de chaque route pré-rendue) et la CSP réelle sous Chromium ; les spécificités Safari/iOS restent manuelles. |
...
| **9. `mesure-alignement-app.mjs` est un outil local** | La mesure de l'alignement (ce que tiennent les centures explicites, `.App { text-align: center }` ayant été retirée) n'est PAS dans la CI : elle exige un navigateur installé et un build à jour, et elle n'imprime aucun verdict. | Classé `role: "outil"` dans `guard-proofs.json` : instrument d'atelier rejouable (`npm run build && node scripts/mesure-alignement-app.mjs`), appelé par la procédure écrite dans le commentaire d'alignement de `src/App.css`. Ce sont les gardes — `check-app-chrome.test.js` et `check-prerender-shells.js` — qui prononcent le verdict et empêchent la dérive silencieuse. |

| **10. `mesure-lighthouse-locale.mjs` est un outil d'atelier** | La mesure locale du Speed Index (pile complète : fixture derrière le serveur de rewrites, 3 routes × 3 runs × 2 conditions) n'est PAS dans la CI : elle exige un Chrome installé, coûte ~5 min par condition, et n'imprime aucun verdict. | Classé `role: "outil"` dans `guard-proofs.json` : rejouable (`cd frontend && node scripts/mesure-lighthouse-locale.mjs [--preset desktop]`), avec la recette et les quatre pièges d'outillage mesurés dans `AGENTS.md`. Ce que l'outil ajoute au gate, et que le gate ne peut pas dire : les requêtes `/api/` réellement observées. Relevé du 25/09/2026 (Chrome 152, Lighthouse 12.6.1, condition lue dans les LHR) — mobile `412×823 cpu×4` : SI au meilleur des 3 runs 1 485 ms sur `/`, 843 sur `/jobs`, 882 sur `/contact` ; desktop `1350×940 cpu×1` : 360 / 272 / 258 ms, TBT 0 ms, CLS 0. Mesure de VARIANCE du même jour, sur le MÊME artefact (un seul build) : 30 runs sur `/` mobile → SI 1 026–4 009 ms (σ 828), 20 sur `/contact` → 869–2 674 ms (σ 442), 12 en desktop → 416–854 ms (σ 150) ; le plancher du score sur ces 30 runs est 0,96, sous lequel le seuil du gate (0,9) garde sa marge — détail dans la ligne « Le Speed Index manquant… » des angles morts fermés. **Zéro requête `/api/` en échec** sur les 18 relevés, et 1 `console.error` par run, toujours le même : l'application rapporte la requête de géolocalisation que l'audit bloque lui-même (`*/api/geolocation/available-countries*`, la table de `lhci-cls-budgets.cjs`) — un état attendu, identique en CI. Preuve du contraire (fixture arrêtée, `/jobs`) : deux `502` sur `GET /api/jobs?limit=12&page=1&status=open` et un SI de 953 ms, c'est-à-dire un chiffre juste sur une page fausse. |
| **11. Un harnais de mutation INTERROMPU laisse sa cible mutée, et deux rouges accusent alors le code** | Le harnais (`check-guard-mutations.py`, rejeu manuscrit équivalent) applique la mutation, exécute la preuve, puis restaure dans un `finally` et vérifie l'empreinte SHA-1. Un run tué entre l'application et la restauration laisse donc la cible MUTÉE dans l'arbre partagé — et un rouge parfaitement crédible là où c'est l'outil qui est en cause : le vrai cas rencontré le 25/09/2026 est `.githooks/pre-push` resté sur `if false; then` (mutation « suppression seule bloquée, le remède est refusé »), qui a fait rougir `check-test-environment.js` (« laisse passer le nettoyage qu'il réclame : une suppression seule ») **et** rendu l'ancre du registre périmée (« le littéral à neutraliser apparaît 0 fois »). | Rien dans la CI ne peut le voir : le registre VALIDE les ancres (`pytest backend/tests/test_guard_failure_proofs.py`) mais ne rejoue pas les mutations en permanence, et un branchement qui n'existe plus dans le code ressemble à une dérive du code. Réflexe écrit ici : devant un rouge d'ancre ou de pré-vol qui « ne vise plus rien », comparer la cible à HEAD (`git diff --quiet -- <cible>`) AVANT de toucher au code. Réparation faite à l'identique : `.githooks/pre-push` rendu **octet pour octet** à son blob HEAD, puis `scripts/__tests__/check-test-environment.test.js` **30/30** et `backend/tests/test_guard_failure_proofs.py` **14/14**. |
| **12. Les `@keyframes` : nommées, déclarées — et le décodeur qu'il faudrait pour en faire une règle** | Le garde « aucun sélecteur sans porteur » juge des SÉLECTEURS, jamais un nom d'ANIMATION. Or le nettoyage du 25/09/2026 a retiré trois `@keyframes` orphelines (`fadeIn`, `slideUp`, `loading`) des feuilles source : rien, dans la CI, ne l'aurait vu — et rien n'empêche d'en réécrire une. Le risque est réel dans les DEUX sens : une `@keyframes` que rien ne joue est du poids mort (la feuille décrivant un site qui n'existe pas, exactement le fait de la ligne « L'élagage au build remplacé par un VERDICT »), et un `animation: nom` dont la déclaration a disparu est une animation QUI NE TOURNE PLUS — en silence, sans qu'aucun test échoue, parce qu'un `@keyframes` manquant n'est pas une erreur pour le navigateur. | **Mesuré le 25/09/2026 : les deux directions sont VIDES** sur l'arbre (source ET livré) — aucune `@keyframes` orpheline, aucune référence pendante. `slideInRight` et `slideUp` sont déclarées par les composants qui les jouent (`src/components/ToastContainer.js` injecte la sienne, `src/serviceWorkerRegistration.js` aussi), et l'animation du bandeau de mise à jour ne part même pas : son module est éliminé du bundle (`register()` n'est appelé nulle part, `src/index.js` appelle `unregister()`), donc sa déclaration et son usage disparaissent ENSEMBLE — c'est précisément pourquoi `slideUp` est absent du JS livré tout en étant déclaré dans `src/`. Ce qui manque n'est pas la mesure mais le DÉCODEUR : `animation:` s'écrit aussi en JavaScript (`style={{ animation: … }}`, `animationFillMode`, `animationDelay`), et une sonde écrite à la va-vite sur cet arbre a rendu **3 faux positifs sur 7 noms** (`animationFillMode`, `animationDelay`, un identifiant de variable). Une règle à ce prix-ci coûterait plus de bruit qu'elle ne ferme de trou : elle viendra avec un décodeur conscient du JS, ou pas du tout. |
| **13. La SÉMANTIQUE d'un glyphe de remplacement, et ce que la sonde de contenu ne demande pas** | `e2e/texte-coquille-react.spec.js` prouve que ce que la coquille publie est PEINT par React au même endroit, jamais l'inverse : React peut peindre PLUS dans la même bande (un mot de plus, un bloc asynchrone), et la règle l'accepte. Elle ne juge pas non plus le SENS d'une icône qui remplace un glyphe : sur la première carte de /support la coquille publie « 💬 » (clé `iconSupportRobot` du dictionnaire) pendant que React y peint `lucide-bot` — le nom de la clé dit « robot », sa valeur dit « bulle », et les deux canaux ne racontent donc pas la même chose. Aucune sonde ne peut trancher ça (elle ne connaît pas l'intention) : la décision appartient au plan (`shellIconKey`, src/config/page-sections.js), et c'est un écart CONNU, pas un angle mort qu'on ignore — il est écrit ici pour qu'il ne soit pas redécouvert comme une trouvaille. Ce que la sonde ne couvre pas non plus : le contenu qu'un visiteur SANS JavaScript ne voit pas du tout (une section absente de la coquille et présente chez React) — c'est la direction inverse, tenue par `exigerCorpsDeclare` au build (déclaré vs publié) et par le garde de provenance (publié vs source). |
| **14. Le coût du document pré-rendu n’est plus jugé par un gate de PR — son verdict porte sur la structure** | `e2e/style-layout-document.spec.js` vérifie que chaque coquille pré-rendue publie toujours son corps (nœuds + hauteur, bundle d’entrée bloqué) et PUBLIE le « Style & Layout » de chaque case sans le juger. La raison est mesurée : le premier verdict portait sur le TEMPS (0,6 × à 1,5 × le relevé d’un poste partagé) et le premier passage du runner Linux a rendu 18 des 22 cases rouges sur le MÊME artefact — `/` mobile 662 ms ici contre 80,4 ms là-bas (×8,2), l’écart le plus serré valant 1,85 × (`/payment` desktop : 43 → 23,2 ms). Une borne en millisecondes borne la machine, pas l’artefact : le même document coûte de 1,85 à 8,2 fois moins cher selon l’hôte. | Ce qui reste non couvert, et c’est assumé : une DÉRIVE FINE du coût du document ne fait rougir aucun gate de PR. Ce qui le couvre ailleurs : les budgets Lighthouse du déploiement (job `lighthouse-ci`, `lighthouserc*.cjs`) et les gardes de POIDS (`check-bundle-size.js`, `check-home-shell.js`), dont la grandeur ne dépend pas de la machine qui les lit. Ce qui est jugé ici, lui, est portable : les nœuds sont mesurés IDENTIQUES sur les deux hôtes, la hauteur à 0,9 % près, et la mutation de perte de matière (accueil amputé de ses neuf dernières sections : 279 → 105 nœuds, 7 075 → 1 125 px) fait rougir les DEUX cases de l’accueil. La table est unitairement éprouvée (`scripts/__tests__/style-layout-budgets.test.js`, 8 cas) et deux mutations du registre la prouvent ; une route ajoutée à `src/config/page-meta.js` sans référence fait rougir un cas dédié. Mesure rejouable : `cd frontend && npx playwright test e2e/style-layout-document.spec.js`. |
| **15. La mesure Lighthouse locale n’est qu’une passe MANUELLE** | `frontend/scripts/mesure-lighthouse-locale.mjs` sait montrer ce qu’aucun budget ne dit — la DISPERSION du même artefact sur n runs, les requêtes `/api/` réellement observées, les LHR run par run — mais rien de tout cela ne tourne sans quelqu’un qui déclenche `.github/workflows/lighthouse-mesure-locale.yml` (`workflow_dispatch` uniquement) : aucune PR ne la lance, aucun seuil n’y est asserté, donc une dérive qu’elle seule montrerait reste invisible jusqu’à ce qu’un humain la cherche. Les budgets de la CI, eux, ne dépendent PAS d’elle : `lighthouse-ci` bloque sur `lighthouserc*.cjs`. | Le workflow publie ses LHR en artifact `lhr-locaux` (14 jours, `if: always()`) pour que le run reste relisible sans le relancer, et l’angle est écrit ici plutôt que redécouvert : un rapport qui ne peut pas échouer n’est pas une vérification (c’est la leçon de F9, et celle du Speed Index — mesuré en distribution, gate laissé au score). Le jour où une dérive de DISPERSION devra bloquer, ce n’est pas ce workflow qu’il faudra durcir mais un budget de `lighthouserc*.cjs` qu’il faudra écrire — et il lui faudra un hôte calme, la variance mesurée sur un poste partagé (jusqu’à +44 % au-dessus du minimum) dépassant l’effet cherché. |

| **16. Les formes de verrou de défilement que la règle de forme ne lit pas** | La règle de forme de `src/components/__tests__/barres-rupture.test.jsx` balaie `src/` (137 fichiers lus au 25/09/2026, plancher > 100) à la recherche d'une SEULE forme de verrou global : `document.body.style.overflow =`. Un verrou posé sur `documentElement`, ou porté par une CLASSE (`overflow: hidden` sur `body` par un état d'application, ou le gel par `position: fixed` + `top: -Ypx`, la technique qui conserve la position de défilement), lui échappe entièrement — et le seul juge qui le verrait, la sonde Chromium `e2e/barres-rupture.spec.js`, ne regarde que `/` et que le geste du tiroir mobile : un verrou posé par une AUTRE surface, sur une autre route, ne fait donc rougir personne. | La règle est une règle de FORME (elle refuse une écriture, jamais une liste de fichiers) et sa seconde moitié est mesurée en navigateur, au vrai geste : la mutation « verrou posé sur `<html>` et jamais rendu » fait rougir `e2e/barres-rupture.spec.js` (« 900 px de molette n'ont pas bougé la page : le verrou du tiroir a survécu à sa barre »), donc la classe de défaut est couverte LÀ OÙ la sonde passe. Ce qui reste non couvert est l'autre forme de verrou — et un verrou gelant la page est visible en deux secondes par un humain, ce qui est la raison pour laquelle il est assumé plutôt que gardé au prix d'une heuristique sur du code arbitraire. |
| **17. La QUATRIÈME copie de la liste des langues reste tolérée** | `src/config/languages.js` est le propriétaire unique de la liste offerte par les deux barres, mais `AVAILABLE_LANGUAGES` (`src/services/geolocationService.js`) porte les MÊMES libellés dans une table indexée avec le drapeau, lue par le contrôle de langue de l'inscription (`RegistrationLanguageSelector`) et par la suggestion de langue d'un pays. Un libellé retouché d'un côté ne l'est donc pas de l'autre : la barre et l'inscription pourraient nommer la même langue de deux façons, et rien ne rougirait. | La copie est DÉCLARÉE dans la règle de forme (exemption nommée, avec son motif : table indexée + drapeau, une surface voisine et non la liste de la barre) et l'exemption est REFUSÉE dès qu'elle n'a plus de sujet — mesuré le 25/09/2026 : la table du service neutralisée fait rougir le cas (« l'exemption pour /services/geolocationService.js n'a plus de sujet : elle doit être retirée »). C'est un fait voisin, pas la liste de la barre : son unification est un chantier à part, et le jour où elle sera faite, l'exemption devra disparaître (sinon rouge). |
...
|---|---|---|
| **Composants à état partagé : écouteurs globaux dépendant d'une ref locale d'instance (classe de bug du panneau de notifications)** | 26/09/2026 | Le bug historique du panneau de notifications (`NotificationDropdown.js`) venait d'une architecture à risque : un composant consommant un état partagé (`useNotifications` fournissant `isOpen` et `closePanel`) monté à deux endroits du DOM (barre desktop et barre mobile `md:hidden`), chacun tenant sa propre `panelRef = useRef()` et son écouteur `mousedown` sur `document`. Lors d'un clic dans le panneau ouvert (instance desktop), l'instance masquée recevait l'événement, constatait que `panelRef.current` (son panneau masqué) ne contenait pas la cible, et invoquait `closePanel()` — démontant le panneau avant que l'événement `click` n'atteigne le bouton d'action. Corrigé depuis par un portail unique (`NotificationPanel.js` dans `App.js`) rattaché à une ancre dynamique (`ancre.contains(e.target)`). Pour empêcher cette classe entière de réapparaître sur n'importe quel composant de l'application, `frontend/scripts/check-composants-etat-partage-ecouteurs.js` (règle AST Babel dans `frontend/scripts/composants-etat-partage-ecouteurs.js`, étape de CI `frontend-build`) analyse statiquement l'arborescence `src/` : il identifie les composants qui consomment un état partagé (`useContext` ou hooks personnalisés `use*`), déclarent des refs d'instance (`useRef`), et enregistrent des écouteurs globaux (`document`/`window.addEventListener`) dont le gestionnaire invoque une action partagée conditionnée par `ref.current`. Les états purement locaux (`useState`) et les ancres partagées passées depuis l'extérieur (`ancre.contains`) restent autorisés. Plancher de lecture strict : `MIN_FICHIERS_LUS = 30` (135 fichiers scannés sur l'arbre réel, 0 violation). Preuves : `frontend/scripts/__tests__/check-composants-etat-partage-ecouteurs.test.js` (**7 cas** : cas bugué historique détecté, état local toléré, ancre partagée tolérée, sous-processus sur fixture propre, sous-processus sur fixture buguée avec sortie 1 et nommage précis du fichier, composant, ligne, ref et action, refus du plancher de lecture, et contrôle de l'arbre réel). Deux mutations enregistrées dans `guard-proofs.json` et validées par `check-guard-mutations.py` (détection neutralisée → test rouge ; plancher neutralisé → test rouge). |
| **Contrôles interactifs démontés PENDANT l'appui, avant que le `click` n'arrive (autre moitié de la classe de bug du panneau de notifications)** | 26/09/2026 | La moitié fermée pour le panneau de notifications (ci-dessus) portait sur un écouteur GLOBAL dépendant d'une ref locale dans un composant monté plusieurs fois. La SECONDE moitié est plus générale et ne tenait à aucune architecture multi-instances : sur appareil tactile comme à la souris, l'appui PRÉCÈDE le `click` (séquence mesurée `pointerdown → touchstart → touchend → mousedown → mouseup → click`). Un `setState` dans un gestionnaire d'appui (`onMouseDown`/`onPointerDown`/`onTouchStart`) qui retire l'élément pressé du DOM — fermeture d'une surface, bascule `!X`, retour anticipé `if (!open) return null` — supprime l'élément AVANT que le `click` ne parte : l'action portée par `onClick` ne part jamais (« je clique et rien ne se passe »). Le remède est local (ne pas changer d'état qui démonte le contrôle pendant l'appui : déplacer l'action dans `onClick`, ou différer la mise à jour) et n'exigeait donc pas d'architecture particulière — d'où un balayage SYSTÉMATIQUE de TOUS les contrôles interactifs de `src/`. `frontend/scripts/check-appui-demontage-controles.js` (règle AST Babel dans `frontend/scripts/appui-demontage-controles.js`, étape de CI `frontend-build`) analyse chaque composant : il apparie les setters et états `useState`, résout les gestionnaires d'appui (y compris les fonctions locales qu'ils appellent), puis confronte, pour chaque contrôle interactif (élément natif, porteur d'un `onClick`, ou à `role` interactif), les états écrits par un appui porté par le contrôle OU un ancêtre (bouillonnement) aux états qui gouvernent son démontage (test d'un rendu conditionnel `{etat && …}`/`{etat ? … : …}` ENCLOSANT le contrôle, ou retour anticipé du composant). Un élément qui ne porte QUE un gestionnaire d'appui n'est pas un contrôle : il n'a pas d'action `click` à perdre (il reste trouvé comme ancêtre porteur d'appui). La fermeture par prop (motif `ConfirmModal`) n'est pas un état local suivi et reste donc tolérée. Planchers de lecture stricts : `MIN_FICHIERS_LUS = 30` ET `MIN_CONTROLES_ANALYSES = 100` (135 fichiers, 266 contrôles interactifs parcourus sur l'arbre réel, 0 violation). Preuves : `frontend/scripts/__tests__/check-appui-demontage-controles.test.js` (**12 cas** : appui direct, appui d'ancêtre par bouillonnement, retour anticipé, tolérance de l'état non lié, tolérance de la fermeture par prop, contrôle sans appui ignoré, comptage, et le garde en sous-processus sur fixture propre, fixture buguée, sous-plancher fichiers, sous-plancher contrôles, arbre réel). Deux mutations enregistrées dans `guard-proofs.json` et rejouées par `check-guard-mutations.py` (détection neutralisée → fixture buguée non refusée, rouge ; plancher de contrôles neutralisé → arbre sans contrôle vert, rouge). |
| **Une surface ouverte dans une barre survivait au POINT DE RUPTURE (le menu mobile), et la liste des langues était un fait à trois endroits** | 25/09/2026 | Les deux dispositions de la barre de navigation (`hidden md:flex` / `md:hidden`) vivent dans le MÊME DOM : rien n'empêchait un état OUVERT dans l'une de survivre au franchissement. Le centre de notifications l'a payé le premier (deux panneaux, chacun son écouteur `mousedown`) et s'en protège depuis en fermant dès que son conteneur n'est plus affiché ; le MENU MOBILE portait le même défaut, à l'identique et plus cher. MESURÉ (Chromium, 25/09/2026, `e2e/barres-rupture.spec.js`) : menu ouvert à 412×823 puis écran élargi à 1350×940 — c'est ce que fait une ROTATION de téléphone, 823 px de large en paysage — le tiroir restait OUVERT dans une barre `display:none` (hamburger et fond de fermeture invisibles, `document.body.style.overflow` toujours à `hidden`) et **900 px de molette ne faisaient défiler la page que de 0 px**, sans aucune commande visible pour la débloquer ; le retour en mobile retrouvait le tiroir ouvert. Correctif (`src/components/Navbar.js`) : une `ref` sur le tiroir et un écouteur `resize` qui ferme dès que `getClientRects().length === 0` — l'affichage RÉEL, jamais les 768 px de Tailwind, comme `NotificationPanel`. Seconde règle née du même défaut : UN VERROU GLOBAL MEURT AVEC SON PRENEUR (démontage de la barre rendant le défilement — c'est le chemin de l'écran d'erreur, où l'`ErrorBoundary` remplace tout `AppRoutes`) ; sans ce cas, la mutation était VERTE, la re-exécution de l'effet restaurant le verrou à la place du nettoyage. Les AUTRES composants des deux barres ont été jugés un par un : le CONTRÔLE DE LANGUE est indemne et la preuve est un compte, pas une opinion (un seul `<LanguageSelector>` dans `src/`, dans le sous-arbre desktop ; le tiroir publie son `<select>` DANS son menu ; 0/0 en mobile fermé, 0 + 1 en mobile ouvert, 1 + 0 en desktop ; aucun `id` à dupliquer, aucun verrou global, écouteur extérieur conditionnel, et un sous-arbre `display:none` ne peint ni ne capte), la cloche et son panneau étaient déjà unifiés (chantier précédent), et `MobileBottomNav` n'a ni état ni surface à ouvrir. Restait une COPIE du même genre : la liste des langues vivait à trois endroits (table du composant, cinq `<option>` écrits en dur dans le tiroir, codes du contexte), donc une langue ajoutée à la barre du haut n'arrivait jamais dans le tiroir — elle a un propriétaire unique, `src/config/languages.js`, lu par les deux barres (même remède que `src/config/countries.js`), sans changer un caractère du libellé publié. DEUX FAUX VERTS de la règle, rencontrés pour de vrai : un détecteur qui ne cherchait les libellés qu'entre GUILLEMETS ne voyait pas ceux publiés en TEXTE JSX (les `<option>` remis en dur restaient verts), et un détecteur qui les comptait PARTOUT ne distinguait pas une déclaration d'une mention (`geolocationService.js` les nomme dans ses messages de suggestion, donc sa copie tolérée aurait pu disparaître sans que l'exemption devienne périmée) — le critère retenu est la FORME (valeur d'un `name:`/`nativeName:`/`label:`, ou texte JSX). Preuves : `src/components/__tests__/barres-rupture.test.jsx` (7 cas : la règle dans les deux sens, dont « un panneau AFFICHÉ n'est pas fermé », l'écouteur qui n'existe que menu ouvert, la restitution au démontage, la liste lue du propriétaire, et le balayage de forme des verrous) et `e2e/barres-rupture.spec.js` (4 cas en Chromium, molette réelle, plancher de montage `nav a` pour qu'un compte de zéros ne passe pas sur une page non montée). Mutations rejouées, restaurations vérifiées à l'empreinte SHA-1 : garde du menu neutralisée → 1 cas jsdom + 2 cas navigateur rouges ; restitution au démontage retirée → le cas de démontage rouge ; verrou sur `<html>` jamais rendu → « 900 px de molette n'ont pas bougé la page » ; second contrôle de langue affiché sur mobile → rouge nommé ; garde qui ferme une barre AFFICHÉE → cas jsdom rouge ; verrou sans `getClientRects` dans `MobileBottomNav.js` → balayage rouge ; `<option>` remis en dur → règle de forme rouge en nommant `Navbar.js` ; table du service neutralisée → exemption périmée rouge. Non-régression : `npx vitest run` **1033 ✓ / 0 ✗** (74 fichiers), `e2e/barres-rupture.spec.js` **4/4** et `e2e/appuis-exterieurs.spec.js` **3/3** sur l'artefact bâti avec l'URL de la fixture, shell pré-rendu inchangé (**94 227 o**, liste des langues jamais publiée par une coquille : les budgets du document décrivent le même document). Ni le registre `guard-proofs.json` ni la CI n'ont bougé : ces règles vivent avec leurs preuves dans `src/` et `e2e/`, comme `appuis-exterieurs.test.jsx`, et `playwright test` prend tout `e2e/`. |
| **Géométrie du LCP des autres routes pré-rendues : un propriétaire par route, et l'élément ÉLU est celui du propriétaire** | 25/09/2026 | `/` (héros) et `/contact` (introduction) avaient chacune un propriétaire de géométrie, et `e2e/lcp-geometrie.spec.js` mesurait l'AIRE sur les onze routes pré-rendues — mais les classes qui décident de ces aires étaient RECOPIÉES face à face entre `src/pages/*.js` et `vite-plugins/prerender/shells-routes.js` (le commentaire de la coquille de /jobs l'écrivait : « ses classes sont celles de src/pages/Jobs.js, à l'identique »), et deux classes différentes peuvent donner la même aire aujourd'hui puis diverger demain. Les quatre routes visées déclarent désormais leur géométrie dans `src/config/page-sections.js`, lue par les deux canaux : `/jobs` et `/about` (`frameClass`, `titleClass`, `introClass`), `/privacy` (les mêmes + `sectionTitleClass`, `sectionBodyClass`) et `/how-it-works` (`heroFrameClass`, `heroTitleClass`, `heroSubtitleClass`). DÉCOUVERTE DE LA MESURE : sur `/privacy` l'élément LCP est le CORPS d'une section, pas l'introduction — une déclaration qui n'aurait couvert que l'en-tête aurait laissé le vrai élément sans propriétaire. Garde de source `scripts/__tests__/check-lcp-geometrie.test.js` (**8 cas**, 6 mutations sur sources mutées) : la déclaration porte l'identité ET la géométrie, les valeurs sont celles qui ont été mesurées, aucun canal ne recopie une classe, les deux canaux lisent et peignent la déclaration — chaque recherche scopée au BLOC DE SA ROUTE, parce que quatre déclarations portent désormais un `introClass` et qu'une recherche globale masquait la disparition du champ de la page par celle d'une autre (défaut trouvé et corrigé dans... ce garde, et dans celui de /contact). Sonde navigateur `e2e/lcp-geometrie-declaree.spec.js` (**16 cas** = 8 routes × 2 tailles depuis le 26/09/2026) : navigation réelle, UNE candidate horodatée au premier paint, au-dessus d'un plancher d'aire mesuré (30 000 / 60 000 / 70 000 / 25 000 px²), et surtout l'IDENTITÉ — l'élément élu porte EXACTEMENT l'ensemble de classes que le plan IMPORTÉ par la spec déclare pour cette route. Relevé : aires IDENTIQUES avant et après la refonte (35 640 / 36 002 px² pour /jobs, 74 466 / 80 262 pour /about, 84 360 / 86 676 pour /privacy, 30 320 / 32 656 pour /how-it-works), une seule candidate dans les deux canaux — la refonte est GÉOMÉTRIQUEMENT NEUTRE. Preuves d'échec rejouées, restaurations à l'empreinte SHA-1 : coquille de /about qui republie sa classe en littéral → garde rouge ; page /about qui recopie la sienne → garde rouge ; plan privé de `sectionBodyClass` (/privacy) → garde rouge ; et la mutation qui ne peut être vue QUE par l'identité — `select-none` ajouté à la coquille du sous-titre de /how-it-works (même aire, même candidate, même horodatage) → sonde IDENTITÉ rouge pendant que la sonde GÉOMÉTRIE de la même route reste VERTE. Conséquences réparées dans la même passe : `antiClsSkeletons.test.jsx` lit la hauteur réservée dans le plan (les deux canaux la lisent désormais), et `check-contact-lcp.test.js` est scopé à son bloc + ancre de mutation réalignée (elle visait une coquille d'avant l'enveloppe `min-h-screen`, donc ne mutait plus rien). Non-régression : e2e **114/114**, vitest **1006 ✓ / 4 ✗** (rouges étrangers : fixture de `check-prerender-shells`, état des branches, ancre du hook `pre-push`). Ce que la garantie ne couvre pas : les routes NON pré-rendues, et la sémantique d'un glyphe. Réparation née du même chantier, hors LCP : `check-home-shell.js` (règle 8 : chaque classe publiée par le shell doit avoir une règle dans la feuille du build) accusait `class="App"` — le conteneur des fournisseurs React, publié aussi par le chrome des coquilles, qui n'a AUCUN style propre — depuis que la refonte d'alignement a retiré la seule règle qui le « stylait » par coïncidence (`.App { text-align: center }`). Un vrai rouge, mais qui accusait un hook et poussait à réintroduire une déclaration d'alignement retirée pour de bonnes raisons : l'exemption est désormais DÉCLARÉE avec son motif (`CLASSES_SANS_STYLE`) et REFUSÉE dès qu'elle ne tient plus (classe plus publiée, ou classe devenue stylée), la fixture des tests publiant le chrome comme le build réel et laissant `#root` vide quand le corps est vide. 3 cas ajoutés ; garde **exit 0** sur l'artefact réel, ses 28 cas verts. |
| **Le propriétaire de géométrie couvre les HUIT routes pré-rendues, et la sonde d'identité nomme l'élément ÉLU des quatre dernières (`/login`, `/register`, `/forgot-password`, `/support`)** | 26/09/2026 | La passe précédente avait donné un propriétaire de géométrie à quatre routes ; les quatre autres coquilles continuaient de RECOPIER face à face les classes qui décident de leur plus grand texte, entre `src/pages/{Login,Register,ForgotPassword,Support}.js` et `vite-plugins/prerender/shells-routes.js`. **Mesure d'abord** (Chrome 152, sonde des candidates `largest-contentful-paint`, navigation réelle React peint, 412×823 et 1350×940) : sur chacune de ces quatre pages, l'élément élu est un PARAGRAPHE SECONDAIRE, pas le titre — la ligne légale de contact de `/login` (10 848 / 12 448 px²), la notice d'étape de `/register` (10 560 / 13 056), le sous-titre de `/forgot-password` (14 001 / 16 458) et celui de `/support` (16 468 / 9 324), UNE SEULE candidate par route et par taille, horodatée au premier paint. La migration était possible sans classe dédiée : chacune de ces quatre chaînes n'apparaissait qu'UNE fois dans sa page et une fois dans le corps de sa coquille (vérifié par grep), donc elle tient bien dans le plan. Champs ajoutés à `src/config/page-sections.js` — `legalContactClass`, `stepNoticeClass`, `subtitleClass` (/forgot-password) et `subtitleClass` (/support) — puis LUS par les deux canaux (`className={pagePlan.…}` côté page, `class="${…Plan.…}"` côté coquille). Garde de source `scripts/__tests__/check-lcp-geometrie.test.js` étendu de 4 à 8 routes (toujours **8 cas**, les mutations par index intactes : les nouvelles entrées sont APPRENDÉES après les quatre d'origine, et chaque recherche reste scopée au bloc et au corps de sa route). Sonde navigateur `e2e/lcp-geometrie-declaree.spec.js` passée de 8 à **16 cas** (8 routes × 2 tailles), planchers d'aire par route = plus petite des deux tailles mesurées. Relevé de la sonde sur l'artefact : UNE candidate au premier paint sur les 8 routes × 2 tailles, classes de l'élément élu identiques à celles que le plan IMPORTÉ déclare, aires exactement celles de la mesure d'attribution (aucune dérive). Non-régression : `npx vitest run` **1054 ✓ / 0 ✗** (76 fichiers) ; `e2e/lcp-geometrie-declaree.spec.js` **16/16**, `e2e/lcp-geometrie.spec.js` + `e2e/geometrie-coquille-react.spec.js` + `e2e/texte-coquille-react.spec.js` vertes. UN ROUGE observé et NON imputé à cette passe : `e2e/cls-coquille-react.spec.js` — `/jobs` desktop (0,0862 pour un plafond de 0,01, un `<footer>` inséré à t=176 ms) ; le diff de cette passe ne touche ni le corps de `/jobs` ni le pied de page, et `build/jobs.html` est reconstruit à l'identique, donc ce rouge appartient à l'arbre de travail, pas à l'extension LCP. |
| **Les emoji restants des pages pré-rendues remplacés par des icônes SVG** | 26/09/2026 | Après l'accueil (21 glyphes), quatre pages publiaient encore les emoji de leurs clés i18n dans leur corps : les trois cartes d'À propos, les trois étapes plus le séquestre de « Comment ça marche », les six pastilles de /support (deux modes + quatre lignes) et les cinq de /contact (quatre lignes + le repère de la carte de façade), auxquelles s'ajoutent les QUATRE lignes du bloc de contact de l'ACCUEIL — qui lisent les `actions` de /contact, donc les mêmes glyphes. Toutes sont désormais DESSINÉES par le même registre que l'accueil (`src/config/page-icons.js` : `contactCall`/`contactWhatsapp`/`contactSendEmail`/`contactAddress`, `supportRobot`, `howStep1`/`howStep3`, plus les contenus `promiseFindWork`/`promiseConnect`/`promiseSecurePayments`/`escrow` déjà présents) ; les plans déclarent `icone` au lieu de `iconKey`/`shellIconKey`, la page rend `IconePage`, la coquille `svgDeLIcone`, `MapEmbed` reçoit un nom d'icône et non un emoji, et `shells-home.js` publie le glyphe des lignes de contact par le registre (`glypheDeContact`). MESURE ENTRELACÉE (25 à 30 tours, la variante emoji reconstruite DANS le document livré en remettant chaque emoji à la place de son `<svg data-icone=…>`, minimum) : /contact **−49,9 ms mobile (−22,8 %) / −9,8 desktop (−23,0 %)**, /support **−45,7 (−17,9 %) / −10,2 (−20,7 %)**, /about **−13,1 (−5,4 %) / −2,4 (−5,1 %)**, /how-it-works **−13,7 (−4,6 %) / −1,8 (−3,2 %)**, accueil **−60,8 (−12,5 %) / −9,2 (−10,0 %)** pour ses quatre derniers glyphes. Ce que la mesure établit et qu'une seule page n'aurait pas montré : le gain n'est pas un chiffre unique mais ~8 à 10 ms par emoji mobile — une page qui en porte trois n'en gagne qu'une quinzaine, celle qui en porte six en gagne ~46. Deux pièges de protocole, rencontrés : à 15 tours /how-it-works sortait à +0,6 ms (faux nul) alors qu'à 30 tours il donne −13,7 (le nombre de tours fait partie de la mesure) ; et la suite e2e exige un build à l'URL de la FIXTURE — avec un `npm run build` nu, `/public/stats` est injoignable, React garde le repli « 500+ » qui coïncide alors avec la coquille, et l'écart DÉCLARÉ « 500+ » de `e2e/helpers/texte-coquille.js` devient périmé (`texte-coquille-react` rouge sur `/`, ce qui accuse le build et non le code). Non-régression : `npx vitest run` **1054 ✓ / 0 ✗** (76 fichiers) ; `texte-coquille-react` + `geometrie-coquille-react` **44/44** ; `style-layout-document` + `lcp-geometrie` + `lcp-geometrie-declaree` **61 cas verts**, tables de structure mises à jour (nœuds 112→123 / 120→132 / 157→169 / 135→154 / 347→359, hauteur d'À propos 1 691→1 667 px) ; le garde de provenance déclare maintenant ZÉRO glyphe par clé i18n dans le plan (`clesDeGlypheDuPlan` vide) et 36 icônes dessinées, toutes présentes au registre. |
| **Les quatre derniers écrans de compte migrés emoji→SVG (la clôture)** | 26/09/2026 | Après l'accueil et les quatre pages de contenu, il restait les écrans de compte : /login (📜), /register (⚠️ 👤 🔧 📸 ×2 💡 🌍 ▼ 📜), /forgot-password (✉️) et /payment (💼) publiaient encore leurs emoji `*IconKey`. Le plan déclare désormais un NOM d'icône (forme route `*Icon` : `legalNoticeIcon`, `clientIcon`, `stepNoticeIcon`, `workerIcon`, `photoIcon`, `photoTipsIcon`, `countryGlobeIcon`, `countryChevronIcon`, `badgeIcon`, `noJobIcon`), la page rend `IconePage` et la coquille `svgDeLIcone` ; `pageSectionParts` et `iconesDuPlan` reconnaissent le suffixe `Icon` (réservé aux glyphes DESSINÉS, `Key` aux textes). Trois dessins RÉUTILISÉS (`workerIcon` = `categoryPlumbing`, `badgeIcon` = `contactSendEmail`, `noJobIcon` = `promiseFindWork`) ; les composants PARTAGÉS reçoivent le nom du plan en propriété (`ProfilePhotoUpload`, `CountrySelect`). MESURE ENTRELACÉE (25 tours, minimum, variante emoji reconstruite dans le document livré en remplaçant chaque `<svg data-icone=…>` par son emoji) : /login **−56,3 ms mobile (−34 %) / −11,7 desktop**, /register **−176,2 (−65 %) / −34,3** (huit glyphes), /forgot-password **−65,6 (−43 %) / −15,6**, /payment **−47,6 (−30 %) / −10,4**. LEÇON À NE PAS REPERDRE : le PREMIER emoji d'une page coûte à lui seul ~47 à 66 ms mobile (chargement de la police de couleur) — la loi « ~8-10 ms par emoji » mesurée sur /contact ne décrit que les emoji SUIVANTS. Non-régression : `npx vitest run` **1054 ✓ / 0 ✗** (76 fichiers) ; `geometrie-coquille-react` + `texte-coquille-react` **44/44** ; `cls-coquille-react` + `lcp-geometrie` + `lcp-geometrie-declaree` **61 cas verts** (CLS 0,0000, une seule candidate LCP au premier paint) ; tables de structure mises à jour (nœuds 114→119, 203→233, 108→111, 98→101 ; /payment 885→893 mobile ; /register 2946→2947 mobile et 2515→2517 desktop ; aire LCP de /register 10 560→10 048 mobile / 13 056→12 544 desktop) ; les quatre routes rejoignent `ARTEFACT_CHANGE` du test de portabilité (leur relevé CI porte sur l'artefact d'AVANT). |
| **Les gardes de géométrie étaient calés sur UN hôte — deux rouges de la PR #152 les démasquent** | 26/09/2026 | Deux échecs de `E2E Playwright` sur un arbre vert en local (`vitest` 1054 ✓ / 0 ✗) : ils ne venaient pas du code mais de la MACHINE, et c'est la leçon que `e2e/style-layout-document.spec.js` avait déjà tirée sur le TEMPS, restée non tirée sur la GÉOMÉTRIE. (1) `e2e/carte-accueil.spec.js` mesurait « le bloc ne bouge pas à l'appui » en comparant DEUX mises en page : au chargement, les neuf sections basses de l'accueil (différées par `content-visibility: auto` + une hauteur de repli par section, `src/App.css`) sont mises en page avec des constantes d'un poste ; à l'appui, `click()` a amené le bloc dans la vue et les sections se sont POSÉES. Reproduit localement sous une police étrangère (Verdana, sonde d'atelier `_sonde-carte-police.spec.js`) : le bloc de carte se déplace de 99,1 px (1350×940) / 135,3 px (412×823) quand les sections se posent, et le relevé de la CI donne 25,5 px (4 316,8 → 4 342,3 px) — le chiffre exact du rouge. Correction : le parcours amène le bloc dans la vue AVANT de mesurer (le geste du visiteur qui appuie dessus), attend que la mise en page ne bouge plus, et PUBLIE la dérive (`mise en page différée posée AVANT l'appui : 0.0 px`). Preuve du protocole corrigé, sous la police même qui reproduit l'échec (`_sonde-protocole.spec.js`) : dérive publiée 75,3 px (mobile) / 99,1 px (desktop), Δ(appui) **0 px** dans les deux tailles — la propriété mesurée redevient celle qui est annoncée. Ce qui n'est PAS réparable : aucune constante CSS ne rend la hauteur réservée exacte sur un autre hôte (le nombre de lignes repliées n'est pas connaissable avant la mise en page) ; c'est le prix du levier, borné par `style-layout-document` (nœuds + hauteur, plancher à 80 %) — et le commentaire de `src/App.css`, qui disait ces valeurs « VÉRIFIÉES section par section » par cette sonde, dit désormais ce qu'elle vérifie réellement. (2) `e2e/lcp-geometrie-declaree.spec.js` : l'aire de l'ENCRE de l'élément élu suit les polices de l'hôte — mesurée sur le runner Linux, elle vaut jusqu'à −18 % de celle de ce poste (/support desktop 7 616 px² contre 9 324, qui passait tout juste un plancher de 9 000 ; /jobs desktop 30 874 contre 36 002 ; /login mobile 10 290 contre 10 848), à BOÎTE d'élément IDENTIQUE (16 380 px² pour /support des deux côtés — c'est l'encre qui change, pas le cadre). Trois planchers de plus n'avaient que 1 à 3 % de marge sur ce seul hôte (`/forgot-password` mobile : 13 000 pour 13 104 mesurés en CI ; `/login` mobile 2,9 % ; `/register` mobile 8,8 %) : ils bornaient la machine, pas la page. Chaque plancher vaut désormais ≈ 70 % de la PLUS PETITE des quatre mesures connues (deux tailles × deux hôtes — 30 874 / 73 340 / 83 058 / 27 056 / 10 290 / 9 796 / 13 104 / 7 616 px²), les quatre chiffres en commentaire de chaque ligne du tableau, et le commentaire précise ce qu'un plancher NE PEUT PAS faire : sans le bloc déclaré (`_sonde-plancher.spec.js`, feuille injectée dans le `<head>` servi AVANT le premier paint) le plus grand texte restant vaut encore 12 285 px² (/support mobile), 9 025 (/support desktop) et 11 480 (/forgot-password) — un plancher ne distingue donc pas « le bloc déclaré est là » de « un autre texte a pris sa place ». Ce qui le fait, à sa place : l'IDENTITÉ des classes (l'élément élu porte l'ensemble de classes du plan) et `e2e/lcp-geometrie.spec.js`. Preuves et relevés : `npx vitest run` **1054 ✓ / 0 ✗** (76 fichiers), suite e2e **149/149** sur le build à l'URL de la fixture (`carte-accueil` 4/4, `lcp-geometrie-declaree` 16/16, `style-layout-document` 23/23) — les deux cas rouges de la CI rejoués verts. |
| **Ressource TIERCE déclarée dans le premier écran d'une coquille pré-rendue** | 25/09/2026 | Une coquille est le premier écran d'un visiteur sans JavaScript : ce qu'elle déclare est demandé par le préchargeur AVANT React, donc une origine tierce y était un transfert d'adresse IP, une connexion non décidée et un point de panne sur la route critique — et rien ne le regardait. Le trou était réel et non théorique : la carte Google de `/contact` et de l'accueil était rendue d'emblée (~300 Ko d'embed avant tout appui), et le parcours e2e qui a fermé ce défaut prouve le COMPORTEMENT à l'appui, jamais ce que le build PUBLIE — un `<script src>` distant, un `@font-face` sur un CDN ou un `preconnect` vers une régie ajoutés au plugin de pré-rendu ne feraient rougir aucun test de comportement. `frontend/scripts/check-shell-remote-resources.js` (règle dans `scripts/shell-remote-resources.js`, étape de CI `frontend-build` après le build) refuse toute ressource distante : balises qui téléchargent (`iframe`, `script`, `img` avec `srcset` candidat par candidat, `video`/`poster`, `audio`, `source`, `embed`, `object`, `track`, `input`), `link` de `rel` ressource (`icon`, `apple-touch-icon`, `manifest`, `modulepreload`, `preload`, `prefetch`, `preconnect`, `dns-prefetch`, `stylesheet`), `url()` d'un attribut `style`, et le CSS publié par la coquille — blocs `<style>` en ligne ET feuilles liées mêmes-origine, leurs `@import` suivis en relais. Les origines autorisées sont LUES dans `scripts/site-meta.js` (`SITE_ORIGIN`, `API_ORIGIN`) : une migration de domaine édite un fichier, pas six. Trois décisions assumées : un `rel` inconnu est REFUSÉ (c'est la forme qu'aurait le contournement) ; un plancher de 100 ressources lues fait échouer un lecteur cassé (224 mesurées) parce qu'un vert sans lecture est un faux vert ; la NAVIGATION (`a[href]` vers `wa.me`, la fiche Google — c'est le contrat de la carte différée, sans JavaScript compris) et les DONNÉES (`link rel=canonical\|alternate`, les URL absolues d'un JSON-LD, que le navigateur ne demande jamais) ne sont pas des ressources — les compter serait le premier faux positif du garde. Relevé du 25/09/2026 : **13 coquilles, 224 déclarations lues, toutes mêmes-origine, 0 `iframe` et 0 `<img>` publiés**, aucun rel non classé, aucun `url()`/`@import` tiers (le CSS est INLINÉ : 69 432 o sur `index.html`, 0 `url()`), et 189 fichiers de `src/`, `vite-plugins/` et `index.html` sans une ressource distante. Preuves d'échec rejouées, restauration à l'empreinte SHA-1 : verdict neutralisé, appel de lecture neutralisé, plancher neutralisé — trois rouges nommés. Non-régression : les 23 cas de ce garde verts, et l'arbre COMPLET mesuré le 25/09/2026 à **998 ✓ / 4 ✗** — quatre rouges ÉTRANGERS à ce garde (chantiers en cours sur cet arbre de travail : LCP de `/contact`, état des branches distantes, fixture de `check-prerender-shells`, ancre du hook `pre-push`), rappelés ici comme tels pour qu'ils ne soient pas imputés à cette règle. Preuve sur l'artefact RÉEL, en plus des mutations du garde : l'iframe Google republiée dans `build/contact.html` fait sortir le garde en **1** en nommant la coquille, la balise et l'URL, puis la restauration octet pour octet le rend vert. Ce que le garde ne couvre pas : ce que le JavaScript runtime demande APRÈS le bundle (la carte à l'appui appartient au parcours e2e) et une navigation par `<meta http-equiv=refresh>` — la frontière est « les ressources, pas la navigation ». |
| *(mécanisme depuis RETIRÉ — voir « L'élagage au build remplacé par un VERDICT »)* **Élagage du CSS mort retirant des utilitaires VIVANTS (valeurs arbitraires)** | 25/09/2026 | Le critère de `vite-plugins/purge-css-mort.js` découpait le corpus en jetons avec un motif trop étroit : `w-[340px]` s'y coupait en `w-` et `340px`, donc TOUS les utilitaires à valeur arbitraire — `min-h-[44px]` (cibles tactiles), `z-[9998]`/`z-[9999]` (empilement des modales), `max-h-[520px]` (plafond du centre de notifications) — étaient retirés alors qu'ils sont utilisés. Découvert en comparant la feuille construite à la feuille SERVIE : `520px`, `44px` et `9999` sont présents en production et absents du build local — la CI et toutes les mesures locales jugeaient une feuille que personne ne sert (936 règles retirées au lieu de 504). Ni les tests du plugin, ni `e2e/lcp-geometrie.spec.js` (22/22), ni le plafond de 35 % ne pouvaient le voir. **Deux corrections, et il faut les deux** : le motif des jetons accepte les caractères d'une valeur arbitraire, et le critère revérifie le nom dans le TEXTE du corpus avec des bornes de nom (`nomUtilise` : `container` ne compte pas dans `containerRef`). Preuve d'échec rejouée : motif seul retiré → 2 cas rouges ; motif **et** revérification retirés → **5 cas rouges**, dont le cas de bout en bout (le plafond du plugin finit par refuser). Relevé corrigé : **420 règles / 67 932 o ≈ 7,5 %** de la feuille inlinée sur 13 pages (feuille de `/` 75 116 → 69 455 o, mesurée en retirant puis remettant le plugin sur la MÊME source ; document de `/` de l'artefact final : 102 753 o brut, 19 569 o gzip). `scripts/__tests__/purge-css-mort.test.js` : 20 cas. |
| *(mécanisme depuis RETIRÉ — voir « L'élagage au build remplacé par un VERDICT »)* **Élagage retirant une règle VIVANTE parce qu'UN de ses sélecteurs est mort (liste de sélecteurs tranchée en bloc)** | 25/09/2026 | Même famille que la panne précédente, et c'est la plus grave trouvée dans ce chantier. Le critère de `vite-plugins/purge-css-mort.js` agrégeait les noms de TOUTE la liste de sélecteurs : `button, .btn { min-height: 44px; min-width: 44px; touch-action: manipulation }` (`src/App.css`) était donc retirée EN ENTIER — `btn` n'est posé nulle part, et `button` n'a pas de nom à poser. Relevé dans la feuille livrée avant correction : plus **aucune** règle ne portait les deux 44 px, et `touch-action: manipulation` ne survivait que sur `html`. Sur l'index, l'ancien critère retirait aussi `main, .main-content, … { padding-bottom: max(18px, env(safe-area-inset-bottom)) }` (deux variantes) et `button, .btn, [role=button], [type=button], [type=submit] { min-height: 48px; padding-inline: 16px }` — la cible tactile de tout le site et le rembourrage bas des `main`, décidés par des sélecteurs d'ÉLÉMENT et d'ATTRIBUT qu'un recensement de CLASSES ne peut jamais justifier : ils ne sont pas absents du corpus, ils ne se posent pas. **4 règles par page** étaient retirées à tort (≈ 420 o) et rien ne rougissait : la coquille et React lisent la même feuille, donc `e2e/lcp-geometrie.spec.js` restait 22/22 — il compare les deux peintures entre elles, jamais la feuille au code. Correction : lecture **sélecteur par sélecteur** (`regle.selectors`, découpage postcss qui respecte les virgules de `:is(.a,.b)` et de `[class*="a,b"]`) ; une règle ne part que si TOUS ses sélecteurs sont morts, et un sélecteur sans nom reste vivant par définition. Effets mesurés sur `index.html` : 420 règles mortes retirées au lieu de 504, feuille livrée 69 455 o au lieu de 69 248 o (+207 o = +0,3 %, le prix de ne plus supprimer du CSS vivant), les trois règles ci-dessus présentes sur les 13 pages. Preuve d'échec rejouée : remettre le critère agrégé rougit **2 cas** de `scripts/__tests__/purge-css-mort.test.js` (20 cas), dont celui qui élague le VRAI `src/App.css`. |
| *(pour la part « élagage au build » : mécanisme depuis RETIRÉ — voir « L'élagage au build remplacé par un VERDICT »)* **Reliquats du gabarit Vite dans `src/App.css`, pesés un par un** | 25/09/2026 | Le gabarit apportait trois règles (`.App`, `.App-header`, `.App-link`) et les quatre déclarations de typographie recopiées dans le bloc `@layer base body`. Inventaire et verdicts, mesurés avec le découpage du critère d'élagage lui-même sur le corpus livré (74 fichiers, 37 820 jetons) : `.App { text-align: center }` **RETIRÉE le 25/09/2026**, mais seulement APRÈS que la mesure qui la défendait (l'encre déplacée de 2 à 28 porteurs de texte sur chaque route, jusqu'à 543,89 px) ait servi de matière à la refonte — voir la ligne suivante ; `.App-header` (192 o) et `.App-link` (2 règles, 93 o) **RETIRÉES** — 0 pose, donc jamais servies, l'élagage les ôtait à chaque build ; les quatre déclarations de typographie **RETIRÉES d'App.css** — la feuille livrée déclarait `body` neuf fois et `font-family` deux fois (#53 `index.css`, #96 `App.css`, valeurs identiques), et c'est la TARDIVE qui s'appliquait : éditer le fichier de base n'aurait rien changé, en silence. `index.css` redevient propriétaire unique de la police ; App.css ne garde que `overscroll-behavior`, `position`, `min-height` (sans autre porteur) et le fond `#f9fafb` (qui passe devant le `hsl(var(--background))` du thème). Les autres règles du gabarit (`#root`, `.logo`, `.card`, `@keyframes logo-spin`, `.read-the-docs`) sont déclarées NULLE PART : rien à trancher. Garde : `scripts/__tests__/check-app-chrome.test.js` (18 cas, dont 4 sur ces verdicts d'inventaire) ; 3 mutations rejouées — re-déclarer `.App-header`, re-déclarer la typographie dans App.css, retirer `min-height: 100vh` — rougissent chacune leur cas, restauration vérifiée à l'empreinte SHA-1 d'origine. Non-régression : suite e2e **57/57** (feuille changée sous les pieds de toutes les routes) et Lighthouse desktop **100/100** sur `/` et `/jobs`, mobile 99–100, zéro échec `/api/`. |
| **Alignement du site rendu explicite (fin de l'héritage `.App`), sonde retournée** | 25/09/2026 | `src/App.css` ne porte plus `.App { text-align: center }` : le site est à GAUCHE par défaut et chaque bloc centré le DÉCLARE (`text-center`). Ce qui tranche, page par page : l'ACCUEIL l'écrivait DÉJÀ (`src/pages/Home.js`, 13 `text-center`, dont 6 des 7 blocs d'en-tête de section en `text-center mb-12`) ; about / privacy / contact ne déclaraient RIEN — 0 `text-center` — et passent donc à gauche, titres, prose, cartes, listes et labels de formulaires compris. Les cas limites ont été REGARDÉS (captures réelles, pas d'heuristique) : le bloc « Qui sommes-nous » de l'accueil est de la PROSE de deux paragraphes (le centrer était l'erreur de typographie que la refonte retire) et les cartes d'étapes de `/how-it-works` gardent leur titre centré (`mx-auto` sur l'icône + `text-center` écrit) avec un corps à gauche, comme les cartes de `/about` — donc AUCUNE addition n'a été nécessaire : la centure voulue était déjà écrite partout où elle l'est. La sonde `scripts/mesure-alignement-app.mjs` (classée `role: \"outil\"`, hors CI) a été RETOURNÉE sur la nouvelle décision : elle mesure 193 déplacements d'encre tenus par les centures explicites, contre 231 pour la règle héritée, et `/jobs` desktop comme `/privacy` desktop à 0 décideur. Gardes : `scripts/check-prerender-shells.js` exige désormais l'INVERSE d'avant (plus aucune page pré-rendue ne republie une centure par héritage) — indispensable, parce qu'une règle réintroduite recentrerait les DEUX canaux ENSEMBLE et les sondes navigateur, qui comparent les peintures l'une à l'autre, resteraient vertes ; `check-app-chrome.test.js` (18 cas) pin les deux moitiés du dessin. Preuves d'échec rejouées : remettre la règle rougit le garde d'App.css, une page de la fixture qui la republie fait rougir le refus nommé de `check-prerender-shells.test.js`. Non-régression : `e2e/geometrie-coquille-react.spec.js` **22/22** (encre ET `text-align` calculé, coquille vs React), `e2e/lcp-geometrie.spec.js` **22/22** — la sonde de géométrie LCP des 11 routes que cette refonte devait faire rejouer —, suite e2e **57/57**, vitest 976 ✓ / 3 ✗ (les trois rouges pré-existants), Lighthouse desktop **100/100** sur `/` et `/jobs`, mobile 100 (`/jobs`, `/contact`) et 98–99 (`/`), feuille inlinée 69 455 → 69 432 o (−23 o, exactement la longueur minifiée de la déclaration retirée, relevé sur `build/index.html`). |
| **« Le document lui-même » comme dernier levier de performance (≈ 0,4 pt)** | 26/09/2026 | *(L'élagage au build cité ci-dessous comme le moyen de la réduction a depuis été RETIRÉ et remplacé par un verdict — voir la ligne « L'élagage au build remplacé par un VERDICT ».)* Question posée : les 75 558 o de CSS inliné dans chaque page (836 règles, parse + mise en page) sont-ils ce qui manque aux routes qui n'atteignent pas 100 de moins d'un demi-point ? Réponse en deux temps. (1) **Réduit** : `vite-plugins/purge-css-mort.js` retire les règles qu'aucun élément de l'artefact livré ne peut satisfaire. Relevé CORRIGÉ du 25/09/2026 (le critère a été réparé entre-temps — voir la ligne « Élagage du CSS mort retirant des utilitaires VIVANTS » : les chiffres publiés ici la veille, 78 règles / 7 844 o / 10,4 %, décrivaient une feuille qui n'est pas celle servie) : **504 règles, 75 720 o ≈ 8 % de la feuille inlinée** sur 13 pages (les familles héréditaires des packs, qu'aucun JS/JSX ne pose : `.auth-page`, `.bottom-nav`, `.job-card`, `.install-banner`, `.splash-shell`…). Deux refus dans le build (corpus illisible ; élagage > 35 % d'une page), garde `scripts/__tests__/purge-css-mort.test.js` (**15 cas**), et `e2e/lcp-geometrie.spec.js` → **22/22, aires identiques** coquille/React sur les 11 routes : la réduction ne change pas un pixel (suite e2e entière : 57/57). (2) **Mesuré** sur l'artefact corrigé, plugin retiré puis remis à réglages identiques : document 108 856 → 102 546 o brut (−5,8 %), 20 336 → 19 209 o gzip niveau 6 (−5,5 %), feuille inlinée 75 565 → 69 255 o. Les mesures de Lighthouse publiées la veille (`Parse HTML & CSS` 33 → 30 ms, `Style & Layout` 552→554 ; 243→232 ms) ont été prises sur l'artefact du critère cassé : **à refaire**, et la feuille corrigée étant plus grande, l'effet réel ne peut qu'être plus petit. Le **SI et le score ne bougent pas** : le Speed Index est la progression visuelle SIMULÉE (il ne contient pas le parse), et l'écart d'un run à l'autre sur ce poste atteint plusieurs secondes pour le MÊME artefact : sur 30 runs du même artefact (25/09/2026), le SI de `/` mobile va de 1 026 à 4 009 ms (σ 828 ms, étendue 2 983 ms) — trois ordres de grandeur au-dessus de l'octet gagné. **Piste fermée par la mesure** : les runs qui manquent 100 de moins d'un demi-point (score exact 0,996 = 0,4 pt sur `/`, 0,999 = 0,1 pt sur /contact) le manquent sur le SI, c'est-à-dire sur la variance de l'hôte, pas sur le document — distribution complète dans la ligne « Le Speed Index manquant… » ci-dessous. Corollaire : la question ne se repose PAS sur cet hôte, dont le bruit dépasse largement l'effet cherché. |
| **L'élagage au build remplacé par un VERDICT (« aucun sélecteur sans porteur »)** | 25/09/2026 | `vite-plugins/purge-css-mort.js` a été **supprimé**, pas corrigé, pour trois raisons mesurées : (1) il **masquait la dette** — la feuille servie différait des sources, et l'on ne pouvait plus lire dans `src/` ce que le site servait ; (2) il ne jugeait qu'à la **règle entière**, donc les noms morts écrits à côté de sélecteurs VIVANTS lui échappaient : **105 noms de classes, 4 769 o = 6,85 %** de la feuille servie, relevé le 25/09/2026 ; (3) une suppression silencieuse au build ne laisse aucune trace en revue. Les feuilles source ont donc été élaguées **une fois, à la main** : 36 règles mortes, 131 sélecteurs qu'aucun élément ne pouvait satisfaire, 148 arguments morts de `:is()`/`:where()` — `src/App.css` et `src/styles/*` passent, **mesuré contre HEAD**, de **30 676 à 19 636 o** (les sept feuilles source, `src/index.css` comprise, de 33 215 à 22 175 o), et la feuille inlinée servie de **69 432 à 60 929 o** (−8 503 o **par page**). Ce qui remplace l'élagage : `scripts/check-css-selecteurs-morts.js` (règle dans `scripts/css-selecteurs-morts.js`) rend **deux verdicts**, et il en faut deux — (1) les feuilles SOURCE, nom par nom : fichier, ligne, nom mort, sélecteur, et la distinction qui commande la réparation (position **obligatoire** : le sélecteur ne peut rien peindre ; **alternative** de `:is()`/`:where()` : l'argument se retire) ; (2) la feuille LIVRÉE, règle par règle : une règle servie dont **aucun** sélecteur n'a de porteur, c'est-à-dire l'utilitaire qu'un composant éliminé du bundle fait générer à Tailwind — mesuré le 25/09/2026, la branche `import.meta.env.DEV` de `src/pages/Dashboard.js` produisait `bg-purple-600` et `hover:bg-purple-700`, **199 o par page**, invisibles par tout autre moyen. Le corpus des poseurs exclut délibérément les `.css` du build (une feuille ne peut que se justifier elle-même) ; **trois planchers de lecture** (20 fichiers et 200 jetons de corpus, 6 feuilles et 60 règles source, 10 pages) font refuser de juger plutôt que rassurer. Relevé : 7 feuilles source et 13 pages lues, 9 486 règles livrées confrontées à un corpus de 73 fichiers / 37 508 jetons, **0 divergence** — et `Dashboard.js` ne cite plus l'utilitaire que Tailwind générait (Tailwind relève **aussi** les classes citées dans les commentaires : le commentaire qui expliquait le retrait ressuscitait l'utilitaire). Preuves d'échec rejouées : `.job-card-mobile` réécrit dans `src/App.css` → sortie 1 nommant `App.css:244` et la position obligatoire ; un nom mort dans un `:is()` d'un pack → sortie 1 nommant l'alternative ; les utilitaires DEV réintroduits → **build rejoué**, sortie 1 « règle servie sans porteur » sur `about.html`, retour à 0 après restauration ; **5 mutations** du registre (verdict neutralisé → 2 cas rouges, lecture des feuilles source neutralisée → 1, chacun des trois planchers neutralisé → son refus nommé), chacune restaurée à l'empreinte SHA-1 d'origine. Test : `scripts/__tests__/check-css-selecteurs-morts.test.js` (**24 cas**, dont trois arbres illisibles à un seul niveau — sans quoi un seul plancher était démontré). Non-régression mesurée : `e2e/lcp-geometrie.spec.js` **22/22**, `e2e/lcp-geometrie-declaree.spec.js` **8/8**, `e2e/geometrie-coquille-react.spec.js` **22/22** (aires du LCP identiques : la feuille est plus petite, pas différente). Étape ajoutée au job `frontend-build`. |
| **Le coût du DOCUMENT pré-rendu (« Style & Layout » route par route) — le CSS n’était PAS le coût** | 25/09/2026 | Question posée APRÈS l’élagage : le HTML pré-rendu (94 227 o servis sur `/` = 60 929 o de feuille en ligne pour 811 règles PLUS 33 298 o de document hors feuille — le « 33 ko » de la question —, 279 nœuds, 7 075 px de haut pour un écran de 412 px) et le « Style & Layout » de 552 ms relevé par Lighthouse sont-ils le levier qui manque ? `e2e/style-layout-document.spec.js` (22 cases = 11 routes × 2 conditions, plus la couverture EXACTE des routes pré-rendues) lit les compteurs du RENDERER au CDP — `RecalcStyleDuration` + `LayoutDuration` de `Performance.getMetrics` — sur une page NEUVE par relevé dont le bundle d’entrée est BLOQUÉ : c’est le document qui est mesuré, jamais React ; le bridage CPU est posé AVANT la navigation, et la grandeur retenue est le MINIMUM de 3 runs (le bruit d’un poste partagé ne peut qu’AJOUTER du temps : médiane jusqu’à +44 % au-dessus du minimum sur l’accueil mobile). Relevé (minima de 5 runs, mobile 412×823 cpu×4 / desktop 1350×940 cpu×1) : `/` 662/126 ms, `/register` 452/86, `/how-it-works` 344/61, `/contact` 278/46, `/support` 261/51, `/forgot-password` 255/46, `/about` 245/47, `/login` 230/45, `/payment` 220/43, `/privacy` 169/33, `/jobs` 155/30. **Deux pistes fermées, et une part fixe trouvée.** (1) LE STYLE N’EST JAMAIS LE SUJET : 1,3 à 51,8 ms contre 28,6 à 676,1 ms de layout, soit 2 à 6 % du total — les 811 règles du document coûtent ~10 ms là où le layout en coûte 141 à 647 ; l’élagage du CSS était justifié (poids mort, sources menteuses) mais il n’était PAS le coût, et cette ligne existe pour que la question ne se rouvre pas. (2) L’ANCIENNE LECTURE « le coût suit les pixels ou les nœuds » est RÉFUTÉE : balayage de l’accueil tronqué à 1, 2, … 10 sections (minimum de 3 runs) 339 / 366 / 494 / 558 / 558 / 569 / 618 / 628 / 622 / 650 ms — les QUATRE premières portent 86 % du coût, les six dernières (3 800 px, ~40 % du markup) n’en ajoutent que ~90 — et quatre mutations mesurées montrent que le contenu ajouté est quasi GRATUIT, même en tête : +6 000 px vides en haut de `<main>` = **0 ms** ; tout `<main>` republié à la suite (279 → 469 nœuds, 7 075 → 13 848 px) = **×0,99** ; héros publié trois fois en tête = 650 → **672 ms** (+3 %) ; deux premières sections dupliquées = +1 %. Le coût marginal d’un nœud mis en page est donc de l’ordre de 0,5 ms à cpu×4 contre ~2,3 ms/nœud en moyenne : une PART FIXE de 250 à 300 ms sur cette route (chrome, première section, mise en page d’un document qui dépasse l’écran) domine. **CE VERDICT-LÀ A ÉTÉ RÉFUTÉ PAR LA CI, ET C’EST LA MESURE QUI LE RÉFUTE** : borne haute ×1,5 et borne basse ×0,6 du relevé d’un poste partagé, il portait sur le TEMPS — le premier passage du runner Linux a rendu 18 des 22 cases rouges sur le MÊME artefact (run 36188879161 : 18 failed, 123 passed), toutes sur la borne basse : `/` mobile 662 ms sur le poste contre 80,4 ms dans la CI (×8,2), `/contact` desktop 46 contre 23,3, l’écart le plus serré valant 1,85 × (`/payment` desktop : 43 → 23,2 ms). Une borne en millisecondes borne la machine, pas l’artefact : **le verdict porte donc sur la STRUCTURE** — les NŒUDS du document pré-rendu (identiques sur les deux hôtes, mesurés sur les 11 routes) et sa HAUTEUR (à 0,9 % près entre hôtes), avec un plancher à 0,8 de la structure de référence — et le coût reste PUBLIÉ à chaque case sans être jugé. **Preuve d’échec rejouée**, restauration vérifiée à l’empreinte SHA-1 (shell bâti 94 329 o, `997b3d3c1adb…` → amputé 75 601 o, `dbb86205f4d2…`, restauré à l’identique) : accueil amputé de ses neuf dernières sections → 279 → **105 nœuds** et 7 075 → **1 125 px**, `/ — mobile` ET `/ — desktop` rouges, en nommant « 105 nœuds peints, sous le plancher de 223 ». La table est éprouvée à l’unité par `scripts/__tests__/style-layout-budgets.test.js` (8 cas : couverture exacte de `src/config/page-meta.js`, plancher dérivé et strictement ordonné, mutation mesurée qui franchit la borne, non-portabilité du temps verrouillée sur les deux relevés, aucun relevé nul) et par **2 mutations du registre** (borne remontée au niveau de la structure → 2 cas rouges ; plancher neutralisé → 2 cas rouges). **Le levier systématique est APPLIQUÉ (26/09/2026)** : `content-visibility: auto` sur les sections 2 à 10, le héros EXCLU (`:not(:first-of-type)`, garantie LCP), avec un `contain-intrinsic-size` EXACT par section — la hauteur de CONTENU mesurée (hauteur rendue moins 96 px de `py-12` mobile / 128 px de `py-16` desktop) —, mesuré ENTRELACÉ (15 tours, mobile cpu×4) : 419,8 → 319,3 ms, −100,5 ms (−24 %) ; la hauteur du document reste 7 080 px mobile / 4 783 px desktop (référence 7 079 / 4 782, +1 px), vérifiée section par section par `e2e/style-layout-document.spec.js`, là où un repli UNIFORME de 1 000 px portait le document à 10 119 px (+3 040) — les neuf sections mesurent 216 à 866 px, donc aucune constante unique ne convient aux deux tailles d’écran. La règle est neutre pour le LCP (/ reste élu sur son `<H1>`, une seule candidate au premier paint, coquille et React) et pour le CLS (0,0000 sur les deux canaux). **La mesure réconcilie avec Lighthouse**, ce qui est la preuve qu’elle parle du même travail : `/jobs` desktop, document + React = 64 ms ici contre 73–76 ms relevés par l’audit de `lighthouserc.desktop.cjs` ; `/` mobile 724 ms document + React à cpu×4 contre les 570–660 ms de la même famille. **Aucune étape de CI ajoutée** : la sonde roule dans le job `e2e-playwright` déjà existant (`playwright test` prend tout `e2e/`), ~57 s de plus sur ce poste — c’est la raison pour laquelle elle est une sonde permanente et non un second budget Lighthouse. Ce que ces budgets NE disent pas : la part de React après montage, le premier écran peint et le CLS (sondes et tables propres), et la dérive FINE du haut : la borne haute est un garde-fou de catastrophe, écrit comme tel plutôt que de faire rougir ce que la mesure a montré gratuit. |
| **La fixture de `check-prerender-shells` recopiait les coquilles `/login` et `/register` — quatre refus du garde parlaient de la fixture, pas du build** | 25/09/2026 | Deux cas rouges (`la fixture conforme passe`, `laisse un chunk vendor* hors surface`) — tous deux pour la MÊME raison, et le troisième rouge du lot (`check-contact-lcp.test.js`, « la coquille de /contact recopie la géométrie ») était déjà vert, vérifié : le garde refusait **quatre** choses — le bouton de connexion publié en `<div>` là où la page porte un `<button type="submit">` (dont le `min-height: 48px` de `[type="submit"]` décide la hauteur), le bouton Google publié alors qu'aucun `client_id` n'est configuré au build, le placeholder pays lu dans le mauvais scope de traduction (`fr.country` au lieu de `registerT('country')`), et un `<select>` de pays que le contrôle de la page (`CountryDisplay.js`, un `<button>`) a remplacé — mesuré : la coquille du build n'en publie **aucun**, et son contrôle est bien `<span class="truncate text-gray-400">-- Pays --</span>`. Autrement dit le garde et le build étaient d'accord ; c'est la **fixture** qui avait dérivé en silence, recopiée à la main pendant que le build changeait. **Correction** : les deux coquilles sont maintenant composées par le module du build lui-même (`vite-plugins/prerender/shells-routes.js`, appelé avec les mêmes entrées que son propriétaire `prerender-route-meta.js` — échappeur à 4 caractères et non l'`escapeHtml()` de `src/utils/htmlEscape.js`, qui échappe AUSSI l'apostrophe, ce que la coquille ne publie jamais). Une source unique ne peut pas dériver de son build. Relevé : `scripts/__tests__/check-prerender-shells.test.js` **46/46**, dont tous les cas négatifs (chacun exige encore son refus nommé et la sortie 1), et `scripts/check-prerender-shells.js` sur l'artefact réel **exit 0**. Leçon générale écrite dans l'en-tête du fichier : une fixture qui recopie un fragment du build est une fixture qui rougira au premier changement de celui-ci, pour une raison qui n'existe pas. |
| **Le Speed Index manquant (« / » à 98–99 au lieu de 100) : la variance de l'hôte, chiffrée** | 25/09/2026 | Question posée : ce demi-point manquant vient-il du DOCUMENT ? Fermée par une mesure qui a le pouvoir de trancher — un seul `npm run build`, puis des dizaines de runs du MÊME artefact (aucun rebuild entre les runs). Sur `/` mobile `412×823 cpu×4`, **n = 30** : SI min 1 026 / p25 1 337 / médiane 1 820 / p75 2 606 / p90 3 151 / max 4 009 ms, étendue 2 983 ms, σ 828 ms, score 96–100 avec **15 runs sur 30 à 100** ; sur `/contact` (n = 20) 869 / 965 / 979 / 1 209 / 1 590 / 2 674 ms, σ 442, 18/20 à 100 ; sur `/` desktop `1350×940 cpu×1` (n = 12) 416 / 435 / 473 / 691 / 724 / 854 ms, σ 150, 100/100 sur les 12 runs. Trois conclusions, toutes mesurées. (1) **Le défaut de 100 est un TIRAGE, pas une propriété du site** : le minimum par condition est stable (1 026 / 869 / 416 ms) quand le maximum vagabonde (4 009 / 2 674 / 854), le bruit ne pouvant qu'AJOUTER du temps — donc c'est le meilleur run qui décrit le coût propre de l'artefact et c'est lui qu'on cite. (2) **Ce n'est pas la page, c'est le BRIDAGE** : même page, mêmes octets, même hôte, même session — entre cpu×4 et cpu×1, σ est divisé par 5,5 et l'étendue par 6,8 ; la famine CPU amplifie le bruit, elle ne le crée pas. (3) **Le plus petit effet mesurable ici vaut l'ordre de l'étendue** (~3 s sur `/` mobile, 1 269 ms d'écart interquartile), trois ordres de grandeur au-dessus des ~500 octets de feuille élaguée (≈ 0,4 pt) qui avaient motivé la question : **rien à faire sur le document**. La seule mesure dont le plancher de bruit serait sous l'effet cherché est le TERRAIN (CrUX/RUM) ou un hôte calme. Ce que la distribution CONFIRME au passage : le gate mobile asserte `categories:performance ≥ 0,9` au MEILLEUR des 3 runs (le score contient le SI à 10 %) et son plancher observé (0,96) laisse au seuil une marge (0,06) plus large que le bruit du score (0,04) ; le SI lui-même n'est asserté NULLE PART, et le desktop n'asserte que TBT et CLS. L'outil `scripts/mesure-lighthouse-locale.mjs` imprime désormais cette distribution (n, min, p25, médiane, p75, p90, max, étendue, σ) et la PERSISTE dans `distribution-speed-index.json` à côté des LHR (centiles interpolés linéairement, rang = p × (n−1) ; σ d'échantillon, n−1) : un run isolé ne peut plus être lu comme une propriété de l'artefact. **Zéro requête `/api/` en échec** sur les 62 relevés des trois bras. |
| **Géométrie du LCP divergente sur une route pré-rendue (pas seulement /contact)** | 25/09/2026 | La règle tenait route par route, mais seuls le héros de `/` et l'introduction de `/contact` avaient une sonde en navigateur : une nouvelle page pré-rendue — ou un composant retouché — pouvait faire ré-élire un élément LCP par React sans que rien ne rougisse. `e2e/lcp-geometrie.spec.js` mesure **chaque route de la table `src/config/page-meta.js`** (11 routes, jamais recopiées) à 412×823 et 1350×940, en deux navigations : une où le bundle d'entrée est bloqué (la peinture de la COQUILLE seule), une réelle. Il échoue si la navigation réelle compte plus d'une candidate LCP, si elle n'est plus horodatée au premier paint, ou si React a peint une aire plus grande (tolérance 1 px²). Relevé du 25/09/2026 : **22/22 vertes, aires IDENTIQUES de part et d'autre** (ex. `/` mobile 39 215 px², `/` desktop 117 320 px², `/jobs` desktop 36 049 px², `/contact` desktop 62 524 px²), une seule candidate au premier paint. Preuve que la sonde peut rougir : rétrécir le héros de `build/index.html` (`text-3xl md:text-5xl lg:text-6xl` → `text-xs`) fait apparaître une SECONDE candidate et échoue en nommant les deux géométries. |
| **Contenu des coquilles pré-rendues jugé mot pour mot (et non « couvert par la provenance »)** | 25/09/2026 | Les sondes de géométrie COMPTAIENT les divergences de contenu sans les juger, en déléguant à `scripts/check-shell-text-provenance.js` — un garde qui lit le HTML du build : une coquille pouvait publier « 1. Email » là où React peint « 1. » puis « Email », ou un autre glyphe, sans que rien ne rougisse. `e2e/texte-coquille-react.spec.js` (22 cas = 11 routes × 2 tailles, harnais `e2e/helpers/texte-coquille.js`, bundle d'entrée bloqué) exige : tout texte verbal publié par la coquille présent chez React, phrase entière, dans la même BANDE verticale ET la même ZONE (`main` / chrome) — donc indifférent au découpage du DOM ; et tout glyphe publié avec un pendant PEINT dans la bande (texte identique si React en peint un là, SVG ou image sinon : mesuré, les 4 drapeaux de `/` sont des `<svg>`, les 6 icônes de /support des icônes lucide). Le chrome se compare dans l'ORDRE et non dans la bande, décision mesurée : sa hauteur dépend de celle du corps — sur /jobs la liste asynchrone emporte le pied de page de y≈699 à plus de 1 800 px, et sa bande est occupée par la barre mobile de React. Relevé : **0 manquant sur les 10 routes comparables** (`/payment` redirige : nommée, jamais comptée verte), sauf les deux chiffres de `/` — données de `/public/stats` formatées par `toLocaleString()` (mesuré « 1+ ») contre les replis statiques `stats[].shellText` — DÉCLARÉS avec leur raison et une contrepartie VÉRIFIÉE, les écarts périmés étant refusés. Planchers par zone (corps ≥ 4 textes / 25 mots, chrome ≥ 6 / 8 ; minima mesurés 5 / 35 et 9 / 21) et aucun glyphe toléré dans le chrome (mesuré 0). Preuves d'échec rejouées, restauration à l'empreinte SHA-1 : libellé d'une autre clé dans `About.js`, autre emoji peint par React, libellé du pied de page suffixé dans `app-chrome.js`, `shellText` aligné sur la donnée — quatre rouges distincts, le dernier sur `/` seulement. Non-régression : e2e **79/79**, vitest 976 ✓ / 3 ✗. |
| **CLS par route mesuré EN NAVIGATEUR, avec ses sources et son attribution** | 25/09/2026 | Le budget de la CI (un plafond par route, médiane de 3 runs) dit QU'UNE page a bougé, jamais quel élément ni de quel côté : la coquille et le montage de React ne se réparent pas pareil, et les sondes d'atelier de #19/#20 ne couvraient qu'une route à une taille. `e2e/cls-coquille-react.spec.js` (23 cas = 11 routes × 2 tailles + le refus d'une route sans budget ; harnais `e2e/helpers/cls.js`) mesure le CLS de Chrome avec son algorithme de FENÊTRE DE SESSION (1 000 ms entre deux décalages, 5 000 ms de fenêtre, plus grande fenêtre retenue — la somme brute ferait rougir un artefact sain) sur DEUX navigations, à 412×823 et 1350×940, et publie les sources de chaque décalage (nœud, rectangle avant, rectangle après) avant de conclure. Le budget est LU PAR IMPORT dans `scripts/lhci-cls-budgets.cjs` (deux tables divergeraient en silence) et une route de `page-meta.js` sans plafond mesuré fait rougir un cas dédié, comme `clsAssertionMatrix` le refuse au chargement de la config. Le PREMIER RENDU N'EST PAS GELÉ, contrairement aux sondes de géométrie : le budget couvre le changement d'état du visiteur réel (le formulaire de /register descend de 14 px quand la géolocalisation échoue — 0,0088, la valeur exacte des 27 runs de la CI), et geler l'état mesurerait 0,0000. Relevé : coquille **0,0000 sur les 22 relevés** (0 décalage), React 0,0088 (/register mobile), 0,0296 (/payment mobile, redirection), 0,0023 (/support desktop), 0,0000 ailleurs. Fait mesuré et durable : le montage en lui-même n'enregistre AUCUN décalage (remplacer `#root` crée des nœuds neufs, et Chrome ne compte que le déplacement d'un nœud déjà présent) — vérifié par mutation, un bloc de 220 px inséré côté React laisse les deux bras à 0,0000 ; un budget CLS garde donc les réactions qui SUIVENT le montage. Preuves d'échec rejouées (restauration SHA-1) : ligne « position non détectée » de `Register.js` à `min-h-[180px]` → bras React rouge à 0,0955 / 0,0233 pour 0,015, le `<form>` nommé, bras coquille resté à 0,0000 (attribution) ; animation REFLOW sur `main` dans `src/App.css` → bras coquille rouge le premier (0,1775 / 0,1109 pour 0,01, `<main>` nommé à chaque trame). Non-régression : e2e **102/102**, vitest 976 ✓ / 3 ✗. |
| **Façade de carte de l’accueil : zéro requête tierce avant l’appui, carte montée à l’appui** | 25/09/2026 | Les gardes jsdom (`home-local-seo.test.jsx`, `Contact.test.jsx`) vérifient le DOM mais **ne font aucune requête réseau** : une iframe cachée, montée puis retirée, ou dont l’`src` est posé sans être visible passerait tous les gardes pendant que le premier écran tirerait l’embed tiers (~300 Ko). `e2e/carte-accueil.spec.js` (4 cas = 2 tailles × [parcours réel, coquille sans JavaScript]) mesure en vrai Chromium : **zéro requête de carte avant l’appui** — vérifié par le motif des hôtes tiers ET par la marque `output=embed` de `CONTACT.mapsEmbedUrl`, plus « aucun `<iframe>` dans la page » — puis **la carte à l’appui** : `iframe` dont l’`src` est exactement `CONTACT.mapsEmbedUrl`, document réellement chargé (réponse interceptée et visible dans le cadre), contrôle disparu, une requête tierce partie ; et **le bloc ne bouge pas** (largeur, hauteur, `y` de document identiques à 1 px). Le tiers n’est intercepté qu’À PARTIR de l’appui (sinon la sonde se cacherait ce qu’elle mesure) et le verdict ne dépend donc pas de Google. Relevé : 16 requêtes avant l’appui dont 0 de carte, 1 à l’appui, iframe 380×320 (mobile) / 1216×320 (desktop) — la boîte exacte du contrôle (`h-80`). Le cas « coquille » prouve le récit sans JavaScript : contrôle publié vers la vraie fiche Google, 7 requêtes, 0 de carte, aucun appui (le lien doit rester vrai). Piège mesuré : `boundingBox()` est relatif au viewport et l’appui défile — la boîte se mesure en coordonnées de DOCUMENT. Preuves d’échec (restauration SHA-1) : iframe rendue d’emblée → rouge nommant l’URL partie ; appui sans effet → rouge « la carte n’est pas montée » ; classe de l’iframe en `h-72` → rouge « 320.0 → 288.0 px ». Non-régression : e2e **106/106**, vitest 976 ✓ / 3 ✗. |
| **Carte Google chargée au premier écran de l'accueil** | 25/09/2026 | Le bloc de contact de l'accueil publiait l'iframe `output=embed` en `loading="lazy"` — mesuré : `loading="lazy"` n'empêche pas le navigateur de charger une iframe dès qu'elle approche du viewport, et c'est le LCP de la page (une de NOS peintures) que les octets tiers repoussent dans le graphe Lantern. La coquille et la page publient désormais la MÊME façade que /contact (contrôle déclaré une fois dans `src/config/page-sections.js` : `mapButtonKey` / `mapIconKey` / `mapFrameClass` / `mapControlClass`), l'iframe ne montant qu'à l'appui. `check-home-shell.js` refuse l'embed dans `index.html` et exige le contrôle (mutation rejouée le 25/09/2026 : l'iframe republiée → `exit 1`) ; `src/pages/__tests__/home-local-seo.test.jsx` prouve qu'aucune iframe n'est rendue après montage. |
| **LCP de /contact peint par la coquille** | 25/09/2026 | Sondes des candidates `largest-contentful-paint` (Chrome 152, 412×823 et 1350×940) : une SEULE candidate, à l'instant du FCP, de même aire que le repaint de React. La géométrie du paragraphe d'introduction a désormais UN propriétaire (`src/config/page-sections.js`), refusé par `scripts/__tests__/check-contact-lcp.test.js`, et le fait est mesuré dans `e2e/contact-lcp.spec.js` — qui rougit si la coquille peint un élément plus petit que React (mutation rejouée le 25/09/2026). Budgets après : 100/100/100 mobile et desktop. |
| **Notification qui « refuse de disparaître » (panneau dupliqué, appui perdu)** | 25/09/2026 | Cause RACINE retirée. `Navbar.js` montait DEUX panneaux (barres desktop/mobile, ouverture partagée par le contexte), chacun écoutant `mousedown` sur le document : l'instance dont le conteneur était `display:none` refermait celui de l'autre PENDANT l'appui, le bouton était démonté avant le `click`, et supprimer/marquer lu ne partait jamais — sans requête ni message. Le centre de notifications est désormais rendu **une seule fois pour tout le site** (`<NotificationPanel />` dans `src/App.js`) ; la navbar ne monte plus qu'une cloche (`NotificationBell.js`, déclencheur sans état) qui transmet son conteneur (`ancre`) au contexte, et le panneau s'y rend par portail — il suit donc la barre qui l'ouvre sans mesurer un pixel, et se ferme si cette barre cesse d'être affichée (franchir 768 px retire la barre desktop). Depuis le 25/09/2026, la cloche visible sur un téléphone est celle de la barre de navigation BASSE (`MobileBottomNav.js`, cinquième item, `grid-cols-5`), et l'ouverture se fait alors VERS LE HAUT : le côté est DÉCLARÉ par la cloche (`sens`, propriétaire unique `notificationPanelPlacement.js`) au lieu d'être mesuré, sous une barre collée au bas de l'écran un panneau qui descendrait sortant de la fenêtre. Preuves : `src/components/__tests__/NotificationCenter.test.jsx` (19 cas, dont la séquence réelle `mousedown` → `click`, l'unicité du panneau, son hébergement, la grille à cinq colonnes et le sens déclaré dans les deux sens) et `e2e/notifications.spec.js` (3 parcours Chromium : la suppression, le franchissement du point de rupture — invisible en jsdom qui ne calcule aucune mise en page — et le parcours MOBILE au doigt sur 412 × 823 px, où c'est la barre mobile qui doit gagner). Mutation rejouée le 25/09/2026 : retirer la garde d'ancrage fait rougir le cas jsdom ET le parcours navigateur (`toHaveCount(0)` → `Received: 1`), puis restauration à l'empreinte SHA-1 d'origine. Le parcours mobile s'appuie sur une séquence tactile MESURÉE (`pointerdown → touchstart → touchend → mousedown → mouseup → click`) : le panneau se ferme sur `mousedown` et supprime sur `click`, donc un `mousedown` de compatibilité pris pour un appui extérieur fermerait le panneau et le `click` qui suit le ROUVRIRAIT — c'est l'inversion au second appui qui le dit. Il vérifie aussi que le bord droit du panneau coïncide avec celui de la cloche **et que son BAS touche son HAUT** (la cloche mobile est sur la barre du bas, le panneau s'ouvre au-dessus d'elle — mesuré : cloche à `y=744`, panneau 340 × 258 dont le bas est à 736 = haut de la cloche − `mb-2`, sans sortir par le haut), que le panneau tient dans 412 px par ses deux bords, que la croix est atteignable sans survol et ≥ 44 px, et qu'un appui extérieur ATTEINT sa cible (un AUTRE item de la barre du bas, à quelques pixels de la cloche : le panneau se ferme ET l'écran change). Trois mutations rejouées : appui sur la cloche traité comme extérieur → rouge (`Expected: 0, Received: 1`) ; fermeture extérieure neutralisée → rouge sur l'item visé ; surface captante `fixed inset-0 z-10` remise dans le portail → « intercepts pointer events ». |
| **Appui perdu sous un menu ouvert (surface plein écran captante)** | 25/09/2026 | Le menu de langue et le sélecteur de pays de /jobs se fermaient au clic extérieur par une surface `fixed inset-0` posée au-dessus de la page : elle CAPTE l'appui, donc la commande visée ne le reçoit jamais. Mesuré dans Chromium (`e2e/appuis-exterieurs.spec.js`) : Playwright retentait 55 fois le clic sur le lien « Emplois » de la même barre, arrêté chaque fois par `<div class="fixed inset-0 z-10">` — le premier appui sur n'importe quelle commande de la barre était perdu. Les deux surfaces sont remplacées par un écouteur `mousedown`/`touchstart` sur `document` qui teste la cible contre son conteneur (`contains`) : l'appui atteint sa cible et le menu se referme dans le même geste ; `CountrySelect` n'enregistre plus son écouteur quand sa liste est fermée ; la règle de forme est pinnée sur tout `src/` par `src/components/__tests__/appuis-exterieurs.test.jsx` (**13 cas** : comportement des 3 composants, aucune surface plein écran sans fond, aucun écouteur sans `contains`, cas synthétiques prouvant que chaque règle mord, et décomptes prouvant que le balayage a lu son sujet). Mutation rejouée le 25/09/2026 : remettre la surface captante → **2 cas jsdom rouges** (le balayage nomme le fichier et la ligne) et **2 parcours navigateur rouges**, restauration vérifiée à l'empreinte SHA-1 d'origine. Un fond visible (`bg-black/30`, `bg-black/50`) reste autorisé : un tiroir ou une modale bloque la page derrière, c'est l'intention. |
| **E2E Playwright en CI** | 24/09/2026 | Job `e2e-playwright` ajouté dans `ci.yml` : exécute les parcours utilisateurs contre l'API mock et Vite preview sous Chromium. |
