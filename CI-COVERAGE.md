...
| **E2E Playwright tests (Chromium + Firefox + WebKit)** | Régressions des 9 parcours interactifs, **suite entière sous Chromium headless** (géoloc/GPS, support, formulaires, auth travailleur, disparition d'une notification au clic réel, LCP de /contact, **géométrie du LCP des 11 routes pré-rendues à 412 et 1350 px**), blocage CSP au runtime, plantage du build compilé dans un vrai navigateur. **Les DEUX parcours d'APPUI (`appuis-exterieurs.spec.js`, `notifications.spec.js`) sont en plus rejoués sur Firefox et WebKit** : la séquence d'événements d'un appui tactile (`pointerdown → touchstart → pointerup → touchend → mousedown → mouseup → click`) est un fait de MOTEUR, et le panneau de notifications se ferme sur `mousedown` puis AGIT sur `click` — un moteur qui n'émettrait pas les événements de compatibilité, ou dans un autre ordre, referait la panne « l'appui ne supprime jamais » sans qu'aucun cas Chromium ne le voie. C'est aussi sur Firefox/WebKit qu'un défaut de feuille (carte de /jobs tombée dans l'ErrorBoundary) s'est laissé voir, invisible en Chromium. | Non — exécute la suite E2E réelle contre la fixture API locale (`playtest-api-server.mjs`) et Vite preview. |
