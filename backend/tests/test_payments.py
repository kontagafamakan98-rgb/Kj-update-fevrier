...
def _make_job(client_id, worker_id, proposal_id, amount):
    job = {
        "id": str(uuid.uuid4()),
    ...
    fake_db.jobs._docs.append(job)
    fake_db.job_proposals._docs.append(proposal)
    return job
...
class TestIPNSecurity:
    ...
    async def test_ipn_does_not_trust_payload_status(self, client: AsyncClient):
        ...
        with patch("server.is_paydunya_configured", return_value=True), \
             patch("server.confirm_paydunya_invoice",
        ...
        assert resp.status_code == 200
        payment = await fake_db.payments.find_one({"id": payment_id})
    ...
    async def test_ipn_completes_when_paydunya_confirms(self, client: AsyncClient):
        ...
        with patch("server.is_paydunya_configured", return_value=True), \
             ...
             patch("server.notify_user", AsyncMock()):
            resp = await client.post("/api/payments/ipn/paydunya", json={
    ...
    async def test_disburse_ipn_verifies_with_paydunya(self, client: AsyncClient):
        ...
        with patch("server.check_paydunya_disburse_status",
        ...
        assert resp.status_code == 200
        payment = await fake_db.payments.find_one({"disburse_token": "real-disburse-token"})
...
class TestCheckoutAmountSecurity:
    ...
    async def test_checkout_rejects_non_owner(self, client: AsyncClient):
        ...
        job = _make_job(client1["user"]["id"], worker["user"]["id"], proposal_id, 25000)
    ...
    async def test_checkout_uses_proposal_amount_not_client_input(self, client: AsyncClient):
        ...
        with patch("server.is_paydunya_configured", return_value=True), \
             patch("server.create_paydunya_invoice",
        ...
        if resp.status_code == 200:
            payment = await fake_db.payments.find_one({"job_id": job["id"]})
...
class TestPaymentStatus:
    ...
    async def test_get_payment_status_own_payment(self, client: AsyncClient):
        ...
        with patch("server.sync_payment_status_with_paydunya",
