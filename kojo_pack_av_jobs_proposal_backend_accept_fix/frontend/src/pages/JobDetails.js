...
export default function JobDetails() {
  ...
  const visibleMessages = useMemo(() => {
    ...
    return filterMessagesForPair(messages, user, discussionTarget.id);
  }, [messages, user, discussionTarget?.id]);
  ...
  const loadDiscussionMessages = async (targetId) => {
    ...
    setMessageError('');
    try {
      const messagesResponse = await messagesAPI.list();
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await messagesAPI.send({
        receiver_id: discussionTarget.id,
        content,
      });
      setMessageDraft('');
